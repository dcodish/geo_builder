...
const CELLS: Record<string, CellStatus> = {
  ...
  'distance|segment|line': { status: 'supported', actions: ['claim'], note: 'S5 (ADR-3D-106): claim-gated — the gauge×absolute drive is #386' },
  'distance|segment|plane-run': { status: 'supported', actions: ['drive-dims', 'claim'], note: 'S5 (ADR-3D-106): drives when both operands ride the gauge; PINS THE SCALE (it carries units)' },

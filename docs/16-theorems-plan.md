...
This "stated-vs-derived" line was already drawn by ADR-038 (structural matching on the typed construct + its dependency parents; coordinate coincidences → Phase 9) — this plan extends it to every surface of the feature. *(Two review revisions, §10: detected shapes — kite/cyclic-quad — now DO trigger, B1; and Appendix-(O)/definitions may appear as clearly-separated supplemental content, B0. The table below reads the pre-revision line; §10 is authoritative where they differ.)*
...
- **Tier — AUTHORED per theorem, not computed from premise-completeness** *(operator refinement, 2026-07-03: "when the word diameter appears in the givens, obviously this should be used… but isosceles ⇒ the bisector/median coincidence is only relevant if one of the lines is actually drawn on the diagram")*. Each `TheoremDef` authors its own trigger levels around what, in real bagrut practice, **announces** the theorem:
  - **Key (green)** — the stated given is a near-unambiguous activator, so it surfaces the moment that given lands, even before the full premise exists: a stated **diameter** → 103/104 immediately (a diameter is stated *for a reason*); a stated **tangent** → 105; **two tangents from a point** → 108/109; **⟂ from the centre to a chord** → 97.
  - **Possible (amber)** — an authored *secondary* condition, used sparingly, for theorems that become pointed only in combination. Many theorems have no amber level at all — they are binary: **hidden until their operative object is drawn**. The canonical case: "isosceles" alone surfaces the base-angles theorems (key), but the apex bisector≡median≡altitude coincidence stays **hidden until the student draws a bisector/median/altitude from the apex** — at which point it surfaces green. The operative-object requirement is part of the authored trigger, not a generic "one fact short" computation (which would flood: every stated shape is "one fact away" from dozens of theorems).
  - *Recall* stays **suppressed in v1** (the charter already marks it optional; it is the flood risk).
  - Premise-completeness still matters — it governs the **trace highlight** (what lights up on click grows as the configuration completes), just not the tier.
...
- **Relevancy is NON-monotonic — re-assessed every step** *(operator review B0, 2026-07-03)*: a theorem's tier/rank can **decrease** as later givens arrive, not only increase. A theorem that was headline-relevant on step 3 may become background (or drop off) once step 6 makes a sharper theorem the operative one. `detectTheorems` already recomputes from scratch each step (D4), so this is a **ranking** rule, not new state: rank uses the *current* full fact set, so an entry can be demoted or re-tiered downward between steps. (Guard against flicker with the caps + the collapsed background rows; a demoted entry sinks into its family fold rather than vanishing jarringly.)
...
A sibling module to `src/render/` — a pure consumer importing engine *types and read-only helpers* (`circleMembers`, `pointNeighbors`, `parallelEdgePairs`, `polyEdges`…). If a matcher needs a derived reading the engine doesn't expose, add a **pure helper** to the engine (the `parallelEdgePairs` precedent) — new code, zero behaviour change, guarded by the existing 1862-test wall. ~~`detectShapes` output is **not** an input to matchers (it is conclusion-side); the only shape triggers are shape words the student **typed**~~ *(REVISED by review B1, see §10: `detectShapes` **does** drive shape theorems for detected shapes — kite/cyclic-quad — and the emergent-vs-constructed boundary was RESOLVED 2026-07-04 to **include purely emergent** shapes)*. Typed shape words remain stated-by-definition triggers regardless (their macro decomposition, ADR-110).
...
## 9. Decisions & remaining inputs (updated 2026-07-03 after the operator's follow-up)
...
The ground-truth corpus review returned twelve verdicts; the four pedagogy calls revise this plan as follows (the transcription calls only touched the corpus doc):

- **B0 — relevancy is non-monotonic + Appendix/definitions may appear as supplemental.** (1) A theorem's rank can **decrease** as later givens arrive, re-assessed every step — folded into **D3** (the new "Relevancy is NON-monotonic" bullet). (2) The plan's hard "Appendix (O) items and definitions are structurally impossible to surface" softens to: **Appendix-(O) and definition items MAY appear as clearly-labelled SUPPLEMENTAL/extra content when relevant** — a student won't meet them in the normal headline/background feed, but they can be offered as optional "related / deepen" material (a natural home is the 6c book-link surface, D2 step 4). This revises §2's table row and §7 invariant 4(a)'s "O never enters" to "O never enters the **headline/background feed**; it may appear only in an explicitly-separated supplemental section." The **corpus test still asserts O ∉ the primary feed** — the supplemental channel is out-of-band and not part of `expectSurfaced`.
- **B1 — there is NO "question target" concept; detected shapes drive theorems.** The tool never sees the question text, so the review-card proposal ("a target shape surfaces its theorems only after the proof part lands") is void — *"the user will never reach a point saying what he needs to prove."* Instead, **a shape that is DETECTED in the figure (`detectShapes`) surfaces its theorems** — e.g. a kite's 37/38 fire when a kite is detected (B14, once BDGE is constructed), a cyclic quad's 87 when ≥4 concyclic points exist (B2c). **This revises D5's "`detectShapes` output is NOT an input to matchers."**
  - **Boundary RESOLVED (operator, 2026-07-04): include PURELY EMERGENT shapes.** A detected shape triggers its theorems **regardless of how it was constructed** — including a shape the student never named, recognised only from coordinates (an emergent rhombus between crossing segments, ADR-166/167). The trigger is *"the shape is on the diagram"*, not *"the student typed its vertices."* Rationale (operator's B1 line): the tool never sees the question, so there is no answer to protect; a shape the student can *see* on the canvas is a legitimate premise-side observation, and surfacing its theorem teaches the read-the-figure skill exactly as a typed shape does. This opens D5 fully — **`detectShapes` output (emergent + constructed) is a first-class matcher input.** The §2/Phase-9 stated-vs-derived line still holds for *conclusions* (an instantiated "∠ACB = 90°", a coordinate-coincident concurrency the student can't see) — what changed is that a **visible detected shape** counts as premise-side, not conclusion-side. Corpus entries that assumed constructed-only (B14) still pass (a constructed shape is a subset of "detected"); emergent-shape scenarios (ADR-166 EGFH rhombus) become positive triggers too.
- **B2c — #87 (cyclic quad) surfaces GREEN whenever ≥4 points are stated on a circle** — a drawn quadrilateral is NOT required; `detectShapes` additionally emits **"מרובע חסום במעגל"** for any stated-concyclic set of ≥4 points. Applied in the corpus to B9, B13 (amber → green). Revises §5's circle-block trigger for 87 and the D3 tiering for it.
- **B3c — no sub-question exception is needed.** Since the tool never knows the question, a given-announced theorem that happens to answer a sub-question is fine as-is; the parked "do not surface what a sub-question asks" idea is dropped (it would have required parsing question text, which never enters the system).

...
4. **Corpus ground truth — GROW THE CORPUS** (operator, 2026-07-03: "I put seven questions as an example… if they are actually used in testing and building, I should probably put more"). Q1–Q7 were chosen to exercise the *construct vocabulary* (circle-heavy); the theorem gate needs breadth across the §5 **matcher families**. Operator action: add **~10–15 more real bagrut questions** to `docs/sample questions/`, picked to cover the under-represented families — quadrilateral characterizations, congruence/similarity, midsegment, medians/centroid, parallels+transversal, isosceles — each with (a) the verbal givens, (b) the official figure image, and (c) **the theorems the official solution cites** (the expected-id ground truth). The dev flow stays corpus-driven: 6a can start on Q1–Q7 (circle block first matches their bias); the new questions gate 6b.

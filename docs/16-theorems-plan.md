...
- **B1 — there is NO "question target" concept; detected shapes drive theorems.** The tool never sees the question text, so the review-card proposal ("a target shape surfaces its theorems only after the proof part lands") is void — *"the user will never reach a point saying what he needs to prove."* Instead, **a shape that is DETECTED in the figure (`detectShapes`) surfaces its theorems** — e.g. a kite's 37/38 fire when a kite is detected (B14, once BDGE is constructed), a cyclic quad's 87 when ≥4 concyclic points exist (B2c). **This revises D5's "`detectShapes` output is NOT an input to matchers."** ⚠️ **Boundary to confirm in 6a (scope decision, do not silently pick):** does "detected shape" include a **purely emergent** shape (one the student never named, recognised only from coordinates — the §2/Phase-9 line), or only a shape whose defining points were all **explicitly constructed** (B14's BDGE — every vertex commanded, only the *kite-ness* derived)? The corpus entries assume the latter (constructed objects, derived classification). Resolving this sets exactly how far D5 opens — **flagged for the operator at the 6a feed review**, not decided here.

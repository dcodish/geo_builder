...
**Consequences.** `square/rectangle/rhombus/trapezoid ABCD inscribed …`, `square ABCD with AB = 6`, `triangle ABC with angle BAC = 37`, `square ABCD and segment AC`, `rectangle ABCD with diagonals`, a right triangle inscribed, etc. now **escalate** (the LLM decomposes them, or the student sees "couldn't read that") — none silently draw a wrong figure. Clean canonical forms, order/spacing/filler variants, and the inscribed triangle/quad (named or unnamed centre) still parse. Pinned by `parser-coverage.test.ts` — a 86-case matrix of *parses-to-X* vs *escalates*, both locales — the explicit contract for the boundary. 649 tests green. The guard is intentionally conservative (it errs toward escalating a genuinely-compound utterance); the LLM fallback is what makes that cheap.

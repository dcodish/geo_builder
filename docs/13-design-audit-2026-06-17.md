...
**Wave 3 — R5 DONE; rest NOT STARTED** (each its own focused pass; **all gated by the stability snapshot**):
- [x] **R5 scaffold/merge — SHIPPED** (`da35c96` Pass 1: shared `multiStartSolve` + `solutionAccepted`, behavior-preserving; `a4a2b91` Pass 2 step 1: coupled-block figures added to the snapshot to close the gate gap; `a169744` Pass 2: coupled block merged into `resolveMixedCarriers` with the grid-scan seeding + near-first ported, relative residual + coincide-exempt degeneracy gate, span scaled to actual extent per adversarial review). 3 driven solvers → 2; snapshot unchanged without `-u`; 961 green. `resolveFreeDriven` left as a thin free-only wrapper sharing the scaffold/gate (fully collapsing it into the mixed solver = optional follow-up; would move free figures real→normalised space). Soft-barrier in-search STEERING dropped (relative residual + more restarts + reject gate replace it) — re-add if a scale-free-constraint collapse case appears.
- [~] **R7 binding** — **step (1) SHIPPED** (2026-06-19): the three near-duplicate ancestor walkers (`freeCarrierAncestors` / `freeDrivableAncestors` / `freeCarrierAncestor`) are collapsed into ONE parametrized `ancestors(objects, start, mode, includeStart)` (`'param'` ⊂ `'drivable'`; the singular now skips an already-solving carrier and finds a free one past it — a small correctness gain). **Step (2) SHIPPED**: `reinterpretDiameter` + `reinterpretAsConstraint` now share `driveCoincideOn` (the coincide-driving tail); `replaceCyclicForDiameter`'s antipode pin **kept**. 1033 green. **Step (3) — high-value slice SHIPPED** (2026-06-21, [ADR-045 amendment](06-decisions.md#adr-045)): a new constraint that finds no FREE reachable DOF now SHARES an **over-subscribed** claimed DOF (≥2 carriers = slack) — `ancestors` gained an `includeSolving` flag, `recruitFreeDofs` case (C) re-points one such DOF so the constraint joins the existing joint solve while every prior constraint keeps a carrier. `driveOrCheck` kept as the tie-break. Fires only on the already-failed path ⇒ stability snapshot unchanged (no `-u`); no-slack ⇒ still fails honestly. Fixed two real operator failures (concyclic / `BH=FH` after `HF=4`/`GE=5`). 1093 green. **Still remaining (the harder part):** fully retiring the greedy apply-time pick into one up-front joint decision + the per-DOF stay-put penalty (λ=1e-3 too weak), and the **coupled free-vertex+radius convergence** (bagrut `∠ADB=<number>`, tangled with ADR-064). Gate behind `maxDelta`.
- [x] **R8 — DONE** (2026-06-19): step 1 (shared geometric `otherCrossing`, circle∩circle `avoid`) + **step 2** (signed-side `bySide` geometric key — line∩circle `branch` now selects a stable side along the line dir, not a flipping array index; property test first). Stability snapshot unchanged; 1032 green. (circle∩circle's `cyclableBranch=false` workaround left in place — latent, not a live break.)
- [ ] **R4 Stage 2** — fold the four 0-DOF vertex kinds (`derived`, `perp-offset`, `rotated`, `scaled-offset`; `parallelogram-vertex` stays) into one `DerivedVertex` (NOT a bare `Mat2` — each carries a drivable scalar with per-kind seed/scale). Behind corpus snapshot. **NOT STARTED** — deep/invasive (~44 sites, 6 engine files), zero user-facing; do as its own focused pass per the audit.
- [x] **R9(b) — DONE** (2026-06-19): `circleMembers` now ACCUMULATES across LLM steps (`accrueMembers` folds each built step's on-circle outputs into a running map keyed by circle centre), so "arc BC" resolves to a circle an earlier LLM step created. (`absorb`'s circumcircle coverage was already shipped as R9 part a.) Regression in `llm.test.ts`. 1033 green.

...
**Guards against:** a NEW label in a collinearity statement refusing `references an unknown point` — the operator (session `2je0eg0n`) had to define H first before the line statement worked. **Asserts:** H exists ON line GF, beyond F per the stated order G→F→H, and the subsequent «GH מקביל ל AD» drives clean. Carries `expectViolations: true` for the KNOWN pre-existing #404 morph (asserted to be exactly that one flag) — flipping it clean is #404's acceptance test. Unit coverage: `newlabel-collinear.test.ts` (trailing/leading/interior positions, the unordered set-collinear sibling, <2-anchors honest refusal, all-existing no-theft, drive-the-rider).

...
### `bare-crossing-statement-states-no-label` — «CD חותך את AB» with no point named states the crossing, invents no label (#241, ADR-383)
Operator session `i1mt2us8` (2026-07-21): the unnamed crossing statement escalated to the LLM and died not-understood — and the same sentence later returned a figure with an invented point M («AB חותך את CD» → `M חיתוך AB ו-CD`), a coin flip in what the figure contains. Reading (a): the sentence now lowers deterministically to the point-free `segments-cross` requirement (within both spans, the ADR-166 meaning, the ADR-244 requirement shape) — no label is created, apply improves the default via the ADR-255 reseat, the verifier (`figure.v.segmentsCross`) + `meetsRequirements` + the shared sample core keep every shown configuration crossing, and the ADR-380 forced-crossing dot then offers the naming. Unit locks in `segments-cross.test.ts`.

### `segment-bisection-statement` — «CD חוצה את AB» builds the midpoint + through-line (#240, ADR-382)
Operator session `i1mt2us8` (2026-07-21): segment bisection died not-understood in both orders — חוצה was wired only for the ANGLE sense (ADR-261), whose own comment names segment bisection as the excluded case, and nothing downstream picked it up. Now an ADR-110 macro (`bisectsSegment`): auto-named midpoint of the object segment (ADR-263 `freeLabel`; an explicit «בנקודה K» pins it; an existing midpoint is reused) + `set-line` through the subject (collinear + between) + the ADR-383 crossing requirement, so the default drawing reads as a genuine crossing — never the degenerate all-five-collinear solution of the bare collinearity. Unit locks in `bisects-segment.test.ts`.

### `clicked-crossing-stays-within-its-segments` — a dot-named crossing keeps its within-the-segments meaning (#234, ADR-379)
Prod session `ne810woo` (2026-07-20, the book figure — M,N on BC, apexes A,D, the four cevians): the operator clicked the crossing of two cevians, named it O, changed configuration — and O left the drawn segments while the letter still read as taken. The dot is only ever OFFERED at a crossing interior to both operands, so the gesture states a within-the-ink meet; `markIntersection` lowered it to the bare INFINITE-line crossing, which carries no such requirement (measured: 7 of the 15 displayable seeds put O outside both cevians). The lowering moved to the shared `crossingCommands` seam and now emits the same `onSeg` requirement the typed «AN ו-DM נפגשים בנקודה O» produces (asserted byte-identical), so `meetsRequirements` can never show a configuration where O has left the figure. The seed-sweep oracle runs the check at every displayable seed. Unit locks in `intersections.test.ts`.
...
### `semicircle-bare-adverb-outside` — «BC קוטר חצי מעגל מבחוץ» binds the outside like «מחוץ לריבוע» (#222, ADR-365 Am.)
Operator validation pass 2026-07-20: the object form «מחוץ ל<shape>» bound the bulge (#213's ride-along) but the bare adverbs «מבחוץ»/«בחוץ» (+ «מבפנים» for inside) parsed and silently dropped the stated side. The detectors (single rule, quantified rule, the BULGE_CLAUSE strip) now accept the adverb forms — the ADR-359 Am. 3 synonym set. Unit locks in `unnamed-centre-pickers.test.ts` (#222 block).

### `cross-circle-diameter-new-circle` — «CD קוטר» with the endpoints on two DIFFERENT circles builds the NEW circle on CD (#221, ADR-366 Am.)
Operator play session `id8j4di1` (2026-07-19): «ED קוטר» with the endpoints riding two different circles built nothing; the «במעגל O3» workaround surfaced a visible named centre. The ADR-366 class, membership edition: an unnamed pair beside several circles resolves its host by MEMBERSHIP (one common host → attach; shared members → defer); cross-membership (no common host) routes to `circleOnDiameter` with an auto hidden centre. Unit locks in `endpoint-is-centre-routing.test.ts` (#221 block).
...
### `q27-chords-equal-distances-drives` — bagrut Q27: «EF=EG» claims a DOF though the default draw already satisfies it (#150, ADR-399)

**Guards against:** a consuming constraint committing as an UNOWNED CHECK because the current (deliberately regular) default happens to satisfy it — Q27's default chords are symmetric to machine epsilon, so the equality bound nothing, converged from 1/64 seeds, starved detection to a 1-sample pool, and made the book labeling BE=DE unreachable. **Asserts:** the displayed figure's geometric CONSEQUENCES — F, G bisect their chords (Thales on diameter EO) and AB=CD (equal chords, forced by EF=EG) — beside the stated given. Per-seed health (64/64 re-solve, ~27 displayable, both labelings reachable, no displayable blow-up, the EG=EF slot mirror, the redundant-equality guard) is locked in `src/replay/__tests__/basin-ownership.test.ts`, which also locks #359's every-seed closed form (ADR-400).
### `extend-verb-active-voice` — «מאריכים את הצלע AB עד לנקודה D» builds the same figure as «D על המשך AB» (#350, ADR-403)

**Guards against:** the extension lane being gated on the extension NOUN, so the textbook's active-voice verb register escalated to the LLM (prod, log-triage 2026-07-26) — and, at the engine level, an extension landing D *between* A and B instead of beyond B. **Asserts:** A→B→D collinear in that order with |AD| > |AB|, and the stated continuation B–D actually drawn. Unit coverage: `input-conventions.test.ts` (incl. both Hebrew kaf forms — `מאריכ·ים` medial כ vs singular `מאריך` final ך).
### `plene-spelling-rhombus` — «מעויין ABHD» (plene spelling) builds the rhombus (#389, ADR-405)

**Guards against:** a plene/defective spelling variant of a known noun missing EVERY grammar rule at once — the grammar hard-codes one canonical spelling per noun (~20 sites for «מעוין» alone), so «מעויין» escalated to the paid LLM while the 3-D app's cross-app guidance refers students here with exactly that spelling. **Asserts:** the exact prod utterance parses deterministically and the built figure is a real rhombus (all four sides equal). Unit coverage: `spelling-normalization.test.ts` (both word orders, the עפיפון≡דלתון fold reaching rules that never listed it, the word-bounded «שוה»→«שווה» fold, canonical spellings byte-untouched).
### `newlabel-collinear-rider` — «ישר GFH» with H undefined creates H as a rider on line GF (#402, ADR-408)

**Guards against:** a NEW label in a collinearity statement refusing `references an unknown point` — the operator (session `2je0eg0n`) had to define H first before the line statement worked. **Asserts:** H exists ON line GF, beyond F per the stated order G→F→H, and the subsequent «GH מקביל ל AD» drives clean **with the declared trapezoid preserved** — the `expectViolations: true` #404 ratchet was RETIRED by [ADR-414](06-decisions.md#adr-414) (#412), which gave the created rider its missing `extension` flag so the ∥ slides the referenced rider G instead of morphing the shape; flipping this clean was #404's stated acceptance test. Unit coverage: `newlabel-collinear.test.ts` (trailing/leading/interior positions, the unordered set-collinear sibling, <2-anchors honest refusal, all-existing no-theft, drive-the-rider) + `created-rider-class.test.ts` (the DOF-class flags, ADR-052 sampling, both follow-on statements).
### `implied-circle-membership` — an unnamed membership on a circle-less figure introduces the circle (#362, ADR-409)

**Guards against:** «A ו-C נמצאות על המעגל» / «M מחוץ למעגל» deferring to the LLM on a circle-less figure while the glued pair and «מיתר» already introduced one (the ADR-367 `implied`-discipline adoption gap). **Asserts:** the circle is minted and resolved, both subjects genuinely ride it, and the side form binds the same circle with M outside. Unit coverage: `implied-circle-membership.test.ts` (En mirror, inside-list form, the #186 named-reference seam intact, 2+-circle defer, on-carrier defer, tangent-compound no-theft).
### `midsegment-both-endpoints-anchored-prod-figure` — «DE קטע אמצעים» with both endpoints already on their sides pins BOTH midpoints (#405, ADR-411)

**Guards against:** the silent-drop class — `midsegmentBaseless` handled only exactly-one-anchored, so the both-anchored form fell through to the plain-segment rule: a bare `segment D E`, the given dropped, every row ✓ (masked at seed 0 by the rider sampling at t = 0.5; a 6-ref sweep proved the hole existed since ADR-199's birth). **Asserts:** the operator's full prod figure — D, E driven to their midpoints, DE ∥ BC, and «GE=3DG» genuinely DRIVES F to the quarter point of BC. Unit coverage: `midsegment-405.test.ts` (both-anchored He/En, the midpointOf anchor, the same-host refusal, the zero-anchored bind, the two-triangle and empty-figure escalations, gate transparency on the working forms).
### `midsegment-bare-binds-the-one-triangle` — bare «DE קטע אמצעים» binds to THE triangle; «D על AC» re-seats the default (#405 + #407, ADR-411/412)

**Guards against:** the zero-anchored fall-through (D and E minted as FREE points, E floating off the triangle, all rows ✓) and the default side refusing to yield. **Asserts:** the utterance binds to the figure's one triangle (the ADR-245 definite-reference pattern), and the student's later explicit «D על AC» re-seats the rule-made rider (D = midpoint of AC), E lands on one of the other two sides, DE ∥ the opposite side, the triangle keeps its area.
### `midsegment-restated-side-reseats-never-collapses` — «DE קטע אמצעים במשולש ABC» then «D על AC» re-seats, never flattens (#407, ADR-412)

**Guards against:** the M4 defaults-yield gap — the #71 branch's first-named-side default was baked as an ordinary rider fact, so the explicit «D על AC» STACKED and the solver satisfied `mid(AB) ∈ AC` by flattening the triangle onto one line (area exactly 0, all rows ✓; in prod since 2026-07-14 — before that the form escalated to the LLM). **Asserts:** D is the midpoint of AC (the stated side won), E the midpoint of another side, the triangle's area is real. Unit coverage: `midseg-reseat.test.ts` (re-seat, same-side no-op, free-endpoint variant pin, the 1-anchored student-stated rider never touched).
### `midpoint-membership-contradiction-refuses` — «D אמצע AB» + «D על AC» refused honestly (#408, ADR-413)

**Guards against:** the degeneracy-acceptance class — a constraint system whose only solutions flatten a DECLARED polygon was "solved" by collapse (every residual zero, reported green, no notice). The collapse check rides the ONE step-accept predicate (`stepAccepted`), so every accept path rejects a flattened figure. **Asserts:** the honest over-constrained refusal naming the statement, keep-prior (triangle intact, D still the midpoint of AB). Unit coverage: `collapse-gate.test.ts` (the refusal + the thin-but-legit 3° triangle regression guard).
...
### `unnamed-concurrency-point` — «נקודת מפגש האלכסונים» with no label builds the crossing (#363, ADR-418)

**Guards against:** `specialPointMeet` resolving everything — meet verb, centre family, host polygon — and then bailing for want of a visible NAME, sending a fully determined construction to the paid LLM. **Asserts:** the crossing is built and auto-labelled M, and it really is the square's centre (equidistant from all four vertices). Unit coverage: `unnamed-concurrency.test.ts` (three registers, the conventional letter per family H/I/O/M, named forms byte-identical, never stealing a taken letter or a host vertex, and the ambiguous-host defer).
### `impossible-triangle-refuses-instantly` — «AB = 4, BC = 4, AC = 9» is impossible, not pending (#420, ADR-417)

**Guards against:** a PROVEN metric contradiction reported as an info state («הנתון נרשם אך לא משפיע בינתיים על הצורה») after 27.8 s of ladder work, with the row marked ✗ and the banner saying the opposite. `constraintIsPending` asks whether the residual MOVES — it does here, since the free radius and placement change |AC| — instead of whether it can reach zero, which the triangle inequality forbids forever. **Asserts:** the step is refused with the impossible-message naming the bound (`exceeds 8`), the figure is NOT pending, and keep-prior leaves the 4-4 triangle intact. Unit coverage: `metric-feasibility.test.ts` (the pure check incl. a 4-side quadrilateral, the flat/realisable siblings that must NOT be flagged, and the <8 s wall bound on the operator's sequence).
### `definite-circle-ref-binds-semicircle` — «חסום במעגל» binds THE drawn circle (#430, ADR-422)

**Guards against:** the definite/implicit circle reference being resolved by SOME circle-consuming rules and silently re-created by others, each deciding for itself instead of asking the one resolution seam. After «חצי מעגל», «משולש CDE חסום במעגל» minted a SECOND circle elsewhere and inscribed the triangle in that (C, D, E at 11.7 / 4.0 / 12.3 from the semicircle's centre), `lastError: null`, no violation — while `pointOnCircle` had bound the very same reference correctly. **Asserts:** exactly one circle exists, and C, D, E all ride it at the semicircle's radius. Unit coverage: `circle-reference.test.ts` (the shared `existingCircleRef` seam: named-existing, the single unnamed circle, hidden circles as referents, 2+ circles deferring).
### `second-inscribe-of-existing-points-still-mints` — an all-existing inscribe keeps its own circumcircle (#430, ADR-422)

**Guards against:** over-widening the #430 fix — binding every unnamed «חסום במעגל» to the one existing circle would force a second inscribe's vertices onto the first circle. **Asserts:** after inscribing ABED, «משולש ABC חסום במעגל» still builds a SECOND circle through A, B, C (the operator-locked two-circle reading of `second-inscribed-circle-fresh-centre`). Naming the circle stays the explicit signal to bind instead.
### `semicircle-references-land-on-the-drawn-arc` — a half circle's references land on the DRAWN half (#429, ADR-423)

**Guards against:** the operator's report — "when i draw half a circle, all references to the circle need to be to the drawn half and not the part that isnt shown". A circle's drawn extent was RENDERER-ONLY knowledge (`arc` objects were read at exactly one site, and both decisions fixing which part is drawn were resolved at render time), so the engine's circle was always the FULL circle and E landed at θ = 280° — floating below the diameter, `lastError: null`, no violation. **Asserts:** C, D and E are genuinely on the circle AND on the drawn half. Unit coverage: `arcs.test.ts` (the span helpers, engine↔renderer parity on the drawn extent, the quarter-circle generality, multi-seed confinement that still varies, and the full-circle byte-identical guarantee).
### `arc-midpoint-takes-the-drawn-arc` — «F אמצע הקשת AB» is the midpoint of the arc that exists (#429, ADR-423)

**Guards against:** the sharpest member of the #429 class — a semicircle's endpoints are antipodal, so the arc bisector degenerates and both `between` riders and `arc-midpoint` fell back to an arbitrary perpendicular blind to which half carries ink. 0-DOF and deterministic: the student named arc AB and reliably got the midpoint of the arc that is not drawn. **Asserts:** F sits at θ = 90°, the top of the drawn half, still exactly on the circle.

### `driven-angle-keeps-its-label` — a stated angle that DRIVES the figure is still printed on it (#474, ADR-432)

**Guards against:** the operator's report — the value the whole figure was built around was the one value the figure would not show. Measure labels were derived from the SURVIVING constraint list, so a given the solver CONSUMES vanished from the drawing: here the angle drives G along AD until it holds, leaving no `angle` constraint behind. The wedge arc still drew, because MARKS come from the facts — that asymmetry (mark from the fact, value from the constraint) is the whole defect, and it broke the stated invariant "everything the student stated is visible on the figure" on CLAUDE.md's own flagship interaction. **Asserts:** all three halves together — that the given really is consumed (zero surviving `angle` constraints, so the test cannot pass vacuously if the drive stops happening), that `37°` is printed at B, and that the figure genuinely measures 37° so the label is not a lie.

### `two-secants-vacuous-line-restatements-keep-dof` — ישר re-statements over derived crossings cost no DOF (#432, ADR-424)

**Guards against:** the operator's two-secant power-of-a-point figure (2026-07-30): «ישר ADE»/«ישר ACB» lower to `collinear` constraints that are STRUCTURALLY VACUOUS — E is *defined* as the line∩circle crossing on the line through A,D, and B is coincide-pinned onto the sec-AC crossing — yet each was counted as removing 1 DOF. The shape count clamped to 0 on a figure with one genuine DOF left (the inter-secant angle), the shared sample pool starved to a single drawing, the values panel printed one seed's `CD = 4.12a` / `radius = 4.15a` as forced knowledge, and the cue read "✓ נקבע במלואו". **Asserts:** `freeDofCount === 1`, every step ok, and the genuinely forced closed form holds in units of a = |AB|/10 (AE = 8a, AC = 4a, AD = 5a, BC = 6a, DE = 3a). Unit coverage: `dof-vacuous.test.ts` (the panel's exact printed/withheld row set, the non-vacuous «ישר ABD» control that still consumes a DOF, the midpoint pair-key lane).

### `two-tangent-circles-sizes-bind-touch-named-centre-line-builds` — bagrut Q11: sizes bind the drawn pair, E names the touch, the centre line builds (#538/#539/#541, ADR-438/439/440)

**Guards against:** operator session s0cr31 (2026-08-11) — three defects on ONE figure. (#538) «היקף מעגל O1 הוא 6π» after «שני מעגלים משיקים מבחוץ» minted a PHANTOM circle-O1 beside the drawn pair with a green ✓ — the size lane created its circle directly instead of referencing it, so the #186 naming-by-use binding never ran and the student's stated 6π/81π landed on circles they can't see. (#539) «ישר A O1 E O2 C» minted a duplicate free rider E beside the auto-named touch M, so every later given about E would attach to a phantom node (the 2-D twin of ADR-3D-139). (#541) the collinearity was refused «over-constrained: … cannot hold» though the official figure exists — the drivable-ancestor walk dead-ended at the tangency touch C (circle∩circle behind a Thales aux whose centre is DERIVED — midpoint(O2,B)), so with the radii pinned no reachable DOF remained; the free-radius variant escaped via the radius DOF, which is exactly what masked the gap. **Asserts:** exactly two student-facing circles with r=3/r=9 on the DRAWN pair; no leftover `M` and E genuinely at the touch (|O1E|=3, |O2E|=9, |O1O2|=12); A–O1–E–O2–C collinear in the stated order; both tangencies genuine (radius ⟂ tangent at D and at C).

### `stated-betweenness-adb-holds-through-the-llm-lane` — «ADB»: the stated betweenness is drawn, not its negation (#536, ADR-441)

**Guards against:** operator session s0cr31nw (2026-08-11), the P1 — «ADB» escalated to the LLM, which ALPHABETIZED the stated run to «ישר ABD» (B between A and D — the NEGATION of the student's betweenness), and the seam committed it green over a drawn arrangement it happened to bless. Every LLM-seam honesty gate asked what the decomposition LOST (labels/numbers/relations/verbs/…) or ADDED (#255); none asked whether it REORDERED — yet for a point-run the sequence IS the statement (ADR-050 Am.3). The sequence gate (`restoreStatedSequences`) restores the student's letter order on the canonical lines before the re-parse. The engine needed no change: a delivered order is enforced at every accept gate and flexes free carriers to the stated side; the infeasible order refuses naming the ORDER statement (issue-536-order-blame.test.ts). **Asserts:** the full bagrut-Q11 build + the corrected LLM line «ישר ADB» — all steps ok, A–D–B collinear, and D STRICTLY between A and B.

### `anonymous-circle-binds-by-membership-beside-second-circle` — the anonymous «המעגל» beside a second circle binds by membership (#546, ADR-443)

**Guards against:** prod session fmpqvpwr (log-triage REC-1) — on a triangle + circumcircle + incircle, «משיק למעגל בנקודה B» and the arc family went not-handled and burned paid LLM calls, because the anonymous-reference tail `circles.length === 1 ? circles[0] : null` (present in THREE resolver helpers) gave up the moment a second circle existed. ADR-443: the utterance's named points vote by intersection of their on-circle memberships and a unique host binds; a genuinely-ambiguous reference now ASKS (`ambiguous-circle-ref`, locked in circle-ref-tiebreak.test.ts) instead of escalating. **Asserts:** the full prod-shaped sequence parses deterministically and builds green; exactly two circles (nothing minted beside the pair); one of them is the circumcircle of A,B,C; the bound «קשת AB = קשת BC» genuinely drives the figure (equal arcs ⇒ |AB| = |BC|).

### `diag-shape-noun-binds-declared-kind` — «אלכסוני ה<shape>» binds the ring DECLARED that kind (#770, ADR-457)

**Guards against:** prod session ejvpktqh — «אלכסוני הריבוע נפגשים בנקודה M» on a trapezoid-only figure drew the TRAPEZOID's diagonals with a green ✓ (the resolver bound "the unique quadrilateral", the stated noun read as decoration), and with two quads both diagonal statements deferred to the LLM, which lost them. ADR-457: polygons carry the DECLARED kind (`Polygon.declaredAs`, stamped on every declaration route including the cyclic generic-quadrilateral base), the definite-shape resolver matches the stated noun against it, a named kind with no declared ring refuses BY NAME (`shape-not-found`), and a noun matching exactly one of several quads binds it. **Asserts:** the inscribed-trapezoid + square figure builds; K sits on the trapezoid's diagonals AC and BD; M sits on the square CDFG's diagonals CF and DG; K ≠ M.

### `bare-label-run-declares-quadrilateral` — the bare «Abcd» opener declares the quadrilateral (#505, ADR-444)

**Guards against:** prod session vgrm5pjb — the student's first utterance `Abcd` (no noun, mixed case) was rescued only by the paid LLM, while `abcd` got a scope:unrelated brush-off — inconsistent casings on the same intent. ADR-444: a bare 3–4 letter run of NEW labels declares the obvious shape (ABC → triangle, ABCD → quadrilateral), delegated to the noun rules so the lowering is byte-identical to «מרובע ABCD» by construction; runs over existing points stay statements (the #536 ordered-line lane), 2-letter runs stay segments. **Asserts:** `Abcd` alone builds green; A,B,C,D exist uppercase; the drawn quad is convex.

### `bagrut-gchf-cyclic-tangent-parallel` — right △ABC + GCHF cyclic + AB tangent at F + AB∥GH + diameter from F cuts AC at E (#567, ADR-W-015)

**Guards against:** drift on the first figure certified end-to-end by the **exercise-sequence agent** (operator upload 2026-08-13, played and approved): a real bagrut exercise — right triangle ABC (∠ACB=90°), F/G/H on AB/AC/CB, quadrilateral GCHF inscribed in a circle, AB tangent to it at F, AB∥GH, and the diameter from F cutting AC at E — transcribed from the book image and verified line-by-line through the headless runner, every line deterministic grammar (no LLM step). The exercise states NO magnitudes, so scale and leg proportions stay free DOFs (ADR-052). **Asserts:** all steps ok; ∠ACB=90°; G,C,H,F concyclic around the auto-named centre O; AB tangent exactly at F (OF⟂AB); AB∥GH; E on segment AC and on the diameter from F (F–O–E collinear).

### `parallelogram-altitude-not-false-blocked-771` — «גובה המקבילית מקודקוד B» commits; the parallel gate does not fire inside «מקבילית» (#771, ADR-458)

**Guards against:** prod window 2026-08-17…24 (log-triage) — one user, three submits, all lost. `VERB_GATES.present` was the bare substring `/מקביל|parallel/`, and the Hebrew for PARALLELOGRAM contains the Hebrew for PARALLEL, so every parallelogram utterance read as *stating a parallel relation*; any correct lowering carrying no `parallel`-typed command was judged to have dropped a given and escalated to the paid LLM, which answered `not-understood` for a sentence the grammar had parsed correctly all along (the identical `משולש` twin, same `foot,segment` lowering, committed silently). The gate was unsound both ways: «מקבילית ABCD» escaped only because its type string `"parallelogram"` contains `parallel`. ADR-458 puts the boundary in the lexicon (`HE_END`/`heWord` — JS `\b` is inert around Hebrew letters, so a Hebrew boundary can only be a lookaround) and composes the gate rows from it; `satisfied` became whole-token and now lists `parallelogram` deliberately. **NOTE the lock that bites is `gateProps` in `scenarios-harness.ts`, widened by this issue from one gate to the whole context-free battery** — `parse` SUCCEEDED before the fix, so this scenario alone would have been green while the app refused the step. **Asserts:** both steps ok; AB∥DC and AD∥BC still hold; the altitude's foot F lies on line CD with BF ⟂ CD.

### `copula-less-symbolic-radius-binds-existing-circle-772` — «רדיוס המעגל R» binds R to the drawn circle (#772, ADR-459)

**Guards against:** prod session ah1kqxz5 — on a bagrut circle problem where 12 of 14 submits parsed, «רדיוס המעגל R» was lost to the LLM (which built nothing) and the student RE-DECLARED the whole circle to attach its radius. The radius statement's value slot was number-only without a copula: `setRadius` never needed one, while the symbolic twin demanded «הוא»/«=»/is, so «רדיוס המעגל 5» built and «רדיוס המעגל R» did not — alone among its neighbours. ADR-459 reads the slot once for both value KINDS and states explicitly what the copula had been separating by accident (a noun-first description is still a circle CREATION — the shadow-matrix hard gate caught that mid-fix; a bare circle noun awaits its name; the letter may not be the circle's own centre). **Asserts:** the session's own rows all commit with the workaround removed; exactly one circle, carrying `radiusSymbol: 'R'`; B, C, D on it and A outside; the tangent at C genuinely ⟂ OC; and R parametric — the later `AD = 18R/7` holds in units of the circle's radius.

### `satisfiable-circle-crossing-is-not-refused-830` — «AC חותכת את המעגל בנקודה D» builds, D between A and C (#830, ADR-470)

**Guards against:** prod session j7r7np51 (2026-08-30) — a student building an isosceles triangle with a circle tangent to AB at B was refused the crossing TWICE, with two different and both-false messages («cannot construct D: line chord-AC does not meet circle», then «over-constrained: A–D–C in order on a line cannot hold»), and gave up, finishing on a DIFFERENT figure via «המשך AC» whose R lies beyond C. The order is not merely satisfiable but FORCED: A outside the circle and C inside ⇒ segment AC crosses exactly once, strictly between them, so the refusal asserted something false. Root cause was SELECTION, not the recruiter — the circle's free radius and centre were reachable from D all along (`freeDrivableAncestors(D) = [circle-O, @ctr-O, A]`). The parser sets `avoid: A` from the line's named endpoint, and `otherCrossing` applied its *"keep the root farthest from `avoid`"* rule without checking the precondition its own contract states — **that `avoid` is one of the crossings**. With A outside (|OA|=8.773 vs r=5) it is not, so the rule degenerated into "farthest from an arbitrary point" and placed D at t=1.905, beyond C, violating the order the same command asserts; the order then fought the selection through the joint solve and traded away the tangency. 0 of 40 seeds built it. ADR-470 enforces the precondition and derives the existing `onSegment` SELECTION from a stated `order [X, id, Y]`, so the root is picked on the stated side instead of blind — no new machinery, no solver change. **Asserts:** all steps ok; D on the circle and strictly between A and C (0 < t < 1); the tangency survives the crossing (|OB| = r and OB ⟂ AB); and the isosceles given |AB| = |AC| still holds. Unit locks in `src/engine/__tests__/issue-830.test.ts` cover `otherCrossing` directly (keeps the other root when `avoid` IS a crossing, ignores it when it is not, unchanged when absent) plus the 39/40-seed anti-luck sweep — seed 17 fails on the PREFIX alone and is #855, a separate pre-existing defect.

### `bare-ngon-nouns-build-generic-835` — «מחומש» / «מתומן» build a generic n-gon; «משוכלל» makes it regular (#835, ADR-471)

**Guards against:** prod session khf8ht6c (log-triage 2026-08-30) — a student typed «מחומש תלת מימדי», «מחומש 0», «מחומש» and left with NO figure; that was their whole session. Two defects: the bare noun was *deliberately* excluded (`regularPolygon` returned null without «משוכלל»/regular/an n-gon form — "so a bare pentagon is left to the LLM net"), and «מתומן», the standard modern-Hebrew octagon, was absent from every list — the noun table, the strip regex, `SHAPE_NOUNS_HE`, the span accountant and the geometry-word gate — so the tool answered `scope:unrelated`, telling a student a real geometry word was not about geometry. Operator rulings 2026-09-01: support מחומש/משושה/מתומן; bare ⇒ generic possibly-irregular, «משוכלל» ⇒ regular; every other polygon ⇒ a named not-supported notice that never reaches the LLM; «מצולע משוכלל בעל n צלעות» withdrawn; and *"dont delete capability — if its there and works, leave it"*, so the existing 7/9/10 «משוכלל» builds and «מצולע משוכלל ABCDE» must not regress. **Asserts** the GEOMETRY, which is the ADR-052 half: five vertices, NO hidden circle (the regular route seats vertices on one), and the side lengths NOT all equal — drawing a bare noun regular would assert equal sides and angles the student never stated and would make «משוכלל» meaningless. Unit locks in `src/parser/__tests__/issue-835-ngon.test.ts` (32 tests) cover the three supported nouns and their English mirrors, both octagon spellings, `מתומן` being in scope at all, the named refusals with their offer list, the withdrawn «בעל N צלעות» format, and — the sharpest one — that no existing capability regressed.

### `over-constrained-refusal-attributes-to-the-typed-step-943` — a refusal is attributed to the sentence the student just typed (#943, ADR-487)
...
### `refused-measure-writes-nothing-955` — a REFUSED measure writes nothing on the figure (#955, ADR-491)

**Guards against:** operator, playing round #949 T2 (2026-09-09) — *"despite getting an honest error message that it is impossible, the diagram still put 70 instead of alpha."* «משולש ABC» · «זווית BCA = 50» · «זווית CAB = 70» · «זווית ABC = α» · «α = 70»: the α line was correctly refused (70 + 50 leaves 60 at B) and the figure was drawn right, yet the canvas wrote «70°» at B — the three labels summed to 190° — with `violations = []`, because a label is not a constraint. The symbolic-measure label seam ran at the top of the fold loop, **before** `applyStep`, so a fact that was then refused had already annotated the figure with a number resolved through the whole-list symbol table («α = 70» binds wherever it sits); the other two label sources were always gated on the outcome. ADR-491 moves the seam to the fact's success branch (in-order pass **and** the ADR-104 retry) and adds the verifier check the ruling required — a numeric label must agree with the drawing. Ruling: a refused measure writes **nothing**, not its letter either. **Asserts** the operator's exact sequence: the α row is the refused one (anti-vacuity), B carries no label, A and C keep their true 70° and 50°, the drawn angle at B is 60°, the printed labels sum to 120° not 190°, and the verifier — now checking labels — finds nothing. Unit locks in `src/__tests__/issue-955-refused-label.test.ts` (13), `src/engine/__tests__/label-honesty.test.ts` (11, the check itself with lying labels injected) and `src/replay/__tests__/issue-955-label-check-wired.test.ts` (the check is exercised on every replay).

### `refused-symbolic-length-writes-nothing-955` — the LENGTH lane had the identical defect (#955, ADR-491)

**Guards against:** the length twin of the reported angle case, from the ruling's own class table: «AB = 3» holds, «AB = x» with «x = 8» stated afterwards is refused, and before ADR-491 the segment was labelled «8» while |AB| = 3 — the same pre-apply emission, one lane over. **Asserts** that the surviving given's «3» labels the segment and the refused symbol's «8» does not, so the two lanes cannot drift apart again.

### `blame-lands-on-the-value-line-956` — the refusal names the line that turned the figure impossible (#956, ADR-492)

**Guards against:** operator, playing round #949 T2 (2026-09-09) — *"when alpha cannot be 70 … the failing line should be the alpha=70 and not that the angle is alpha."* On «משולש ABC» · «זווית BCA = 50» · «זווית CAB = 70» · «זווית ABC = α» · «α = 70», the refusal landed on **«זווית ABC = α»** — a statement that is perfectly fine, was green one line earlier, and must not change — quoting a number written on the line *below*, which itself sat green. Root cause: `set-var` lowers to **zero** commands, and blame works on constraints, so a fact producing none can never be blamed; the number reaches the measure through the whole-list symbol table (ADR-483) and the measure takes the fall for a magnitude it did not state. The operator declined all three offered options and gave the rule instead — *"the last command that added the issue"*, i.e. the shortest infeasible prefix, blame its last fact — which also retired the minimal-conflict search from this issue's scope. ADR-492 implements it as an attribution pass after the deferral, poisoning and HOIST have settled: a failing constraint is produced jointly by the row that shaped it and the row that valued its symbol, and blame goes to the **later** of them, so the length lane («AB = x» + «x = 8») is fixed by the same rule. **Asserts** that exactly one group is red and it is «α = 70», that every earlier row stays green, and — the interaction a careless change breaks — that the now-green α-definition still puts **no label** on B, which is drawn at 60°: green does not mean labelled, because ADR-491 emits a label only from a fact's success branch. Unit locks in `src/__tests__/issue-956-blame-later-fact.test.ts` (14) add the banner-quoting path, ordering B as a regression lock (the value typed first leaves the shaping line rightly blamed), a restated value blaming the last one, and a **prefix-sweep oracle** over four sequences that states the ruling executably.

### `inscribed-square-on-a-right-triangle-builds-920` — an inscribed square on a right triangle builds (#920, ADR-493)

**Guards against:** operator report #920 — an inscribed square that USED to draw began refusing «over-constrained: ∠ABC = 90° cannot hold», accusing the student's own right angle, which holds perfectly well (bisected to `499d35c..615e329`). Measured across all 36 configurations (2 shapes × 3 right-angle vertices × 6 variants): 35 build and exactly one refuses — `square@B#3`, reachable only by cycling — so the default the student sees builds at every vertex, which is what this scenario locks. **Root cause of the accusation** (ADR-493 arm 1): `newCons` was `next.constraints.slice(prev.constraints.length)`, the LISTED half only, while this macro attaches all four of its own constraints as SOLVE DIRECTIVES (`driveOrCheck` case 1 embeds without listing). `blameNewStatement` — which exists to name the student's new statement rather than a collateral casualty (#37) — bails on `!newCons.length`, so it was handed an empty list, silently no-opped, and the primary solve's violated set (a prior given) reached the student verbatim. Fixed by `addedConstraints` = listed ∪ driven, reusing ADR-398's `drivenConstraintsOf` rather than adding a second way to ask what a step attached; the refusal now names «|DE| = |EF|», the square's own seating obligation. **Asserts** every step ok, no violations, four placed corners, four equal sides with a right angle at D, and — the given the refusal used to accuse — that the stated right angle at B still holds. The 36-cell outcome map in `src/engine/__tests__/inscribe-joint-solve.test.ts` is the class lock: it replaced a `MARGINAL` set asserting a state that had stopped occurring (so it passed by checking nothing for six weeks), records the one remaining refusal as a **known open gap** held by the operator's 2026-09-09 ruling pending a second case in its class, and fails by construction on any cell change.

### `angle-named-by-its-two-sides-967` — an angle named by its two SIDES lands as a real given (#967, ADR-496)

**Guards against:** prod log-triage 2026-09-10, **2 distinct users / 3 submits** — «מלבן ABCD , אלכסון AB , זוית בין BD לBA היא 30°»; a third user typed the same sentence with Hebrew-letter labels and logged **`not-understood`**, i.e. the paid LLM fallback failed too and that student got nothing. 2-D's angle family (value, Greek symbol, word-numbers, right-angle word, acuteness, bound, range) was reachable through **exactly one spelling** — the three-letter vertex triple — so naming an angle by the two sides that form it, which is how a textbook says it, had no form at all, while 3-D has shipped the mode since #534/#523/#917. **Two plausible diagnoses were measured and falsified first:** the comma splitter is sound («מלבן ABCD , אלכסון BD» builds both clauses), and it is not a spelling slip (every neighbouring spelling failed identically). The decisive measurement was the same line with the angle clause respelled — «מלבן ABCD , אלכסון BD , זווית DBA = 30» built and verified — proving exactly one clause was responsible and that what it needed was a way to **name** the angle, not to phrase a value. Fixed as an **addressing mode** in `angleArms`, the #831/ADR-468 chokepoint that exists to answer "which angle is named" once: BD and BA share B, so the pair resolves to the ordinary ∠DBA triple and every downstream layer is untouched. **Asserts** all steps ok, no violations, and that ∠DBA really measures 30° **on the drawing** — the stated magnitude landed as a given rather than being dropped or matched by luck. The utterance is locked **verbatim** (defective «זוית», no space in «לBA», the ° glyph) because each was a separately plausible point of failure. Unit locks in `src/parser/__tests__/angle-between-sides.test.ts` (26) add eight spellings across both languages, the whole value family inheriting the mode, the by-name refusal of a disjoint pair in two lanes, and the triple / single-vertex / «E בין A ל-B» / «AB מאונך ל-CD» readings asserted unchanged.

### `symbolic-angle-behind-any-copula-969` — a symbolic angle binds behind ANY copula, never its coefficient (#969, ADR-498)

**Guards against:** the P1 honesty breach — «זווית ABC היא 2α» committed **`set-angle value:2`**, the coefficient read as the whole value and the student's α discarded, while the line reported *applied, drawn and verifier-clean*. No honesty gate caught it: a figure asserting 2° for a student who wrote 2α. **Measured before anything changed** (`78440ea`, through the real `parse` and again through the submit path), the hole was **every copula but `=`** — «היא», «הוא», «שווה», «שווה ל-», "is", "equals" — across **every** naming mode and **both** alphabets (`2α` *and* `2x`), so the issue as filed named one of forty-odd cells. **Root cause:** the rule pair split its value read by *vocabulary* — the numeric rule located a number **positionally** (any standalone number, so every copula worked and none was ever written down) while the symbolic rule located its symbol **syntactically**, after a literal `=`; the symbolic rule declined on every other copula and the numeric rule's number-grabbing greed found the `2` of `2α`. Fixed by locating both kinds the **same way** through one reader (`angleValueOf`), which completes for the VALUE half what #831/ADR-468 did for the NAMING half — and a copula list was rejected on measurement, since the numeric lane has no list to copy and the seventh spelling would reopen the hole exactly as the sixth did. **Asserts** all steps ok, no violations, and that pinning «α = 20» afterwards lands ∠ABC at **40°** — the strongest statement of the fix being that the «היא» spelling and the `=` spelling produce the *same figure*. Unit locks in `src/parser/__tests__/angle-copula-value.test.ts` (187) carry the exhaustive naming × copula × value-kind matrix (a seventh copula added there must pass without touching `parse.ts`), the numeric column asserted unchanged, and the negative half: symbolic **bounds** stay bounds (`COMPARES_WITH_SYMBOL`, ADR-390's tripwire in the symbolic alphabet — without it «זווית ABC גדולה מ-α» would have been claimed as an equality), acuteness untouched, both right-angle word forms (`"is a right angle"` strips to a stray article `a` that reads as a variable), bare references, «זוית abc», the angle alias (#267), and an un-nameable angle asserted to **escalate rather than fall back to its coefficient**. The `measureAngle → angle` pair was also **removed** from `shadow-allowlist.json`: the gate had been carrying this defect as an approved shadow, hidden only by rule order.

### `symbolic-angle-copula-matches-equals-969` — the operator's own spelling «זוית A שווה 2α» builds (#969, ADR-498)

**Guards against:** narrowing the fix back to one copula or one naming mode. The operator's scope ruling stated the requirement as a **class** — *"we need to be able to keep the same syntax users use for a regular angle … and alike"*, i.e. every spelling that works for a numeric angle must work for a symbolic one — and named this case themselves. It exercises the **single-vertex** naming lane («זוית A», whose arms are resolved from the figure via `ctx.neighbors`) behind «שווה», a different cell of the matrix from the reported «זווית ABC היא 2α», so it fails for a *different reason* than its sibling if either axis is narrowed. **Asserts** all steps ok, no violations, and ∠BAC at **50°** after «α = 25».
### `diagonal-claim-refused-on-a-side-966` — «אלכסון AB» on a rectangle is refused by name (#966, ADR-499)

**Guards against:** the 2-D half of the operator's #859 ruling — *"the term אלכסון should be sure to be a diagonal and **this is true for all tools**. if the word is used."* 3-D shipped it; 2-D never checked the pair at all, because the role word was stripped as filler beside «קטע»/`segment`/«חבר» and nothing survived to say a claim had been made. Measured before the fix: «מלבן ABCD» then «אלכסון AB» answered **"already drawn"** — worse than silence, because it *affirms* the student's false claim rather than correcting it. **Asserts** the refusal quotes the pair and the shape («AB is not a diagonal of ABCD — it is a side»), that the rectangle itself is untouched and verifies, and that all four vertices are still placed — a refusal keeps the prior figure.

### `diagonal-claim-on-a-triangle-refused-966` — a triangle has no diagonals, and is told so in its own words (#966, ADR-499)

**Guards against:** the second silent wrong, and the one that shows most plainly that the noun was never read as a claim — a triangle has no diagonals **whatsoever**, yet «משולש ABC» then «אלכסון AB» answered "already drawn". **Asserts** it gets a *different* sentence from the side case («AB is not a diagonal — ABC has no diagonals»): the same shared predicate produces both (with n = 3 every pair is adjacent, so it needs no special case in the code), but "AB is a side" would teach nothing to a student who drew it on a triangle, so the two mistakes are explained differently. Numbered separately from its sibling because a refusal case is the easiest thing to leave untested.

### `diagonal-of-nothing-is-caught-by-the-verifier-966` — fresh ink called a diagonal is caught at the end (#966, ADR-499)

**Guards against:** «משולש ABC» · «משולש DEF» · «אלכסון AD» — fresh ink drawn and called a diagonal of nothing, green ✓; 3-D's #859 symptom exactly, in 2-D. It cannot be caught at apply time without breaking a legitimate order, because *"no polygon holds both labels"* is also what the forward declaration looks like at the moment it is typed (see the sibling below). So the claim is **deferred** and settled by the givens verifier against the finished figure — [ADR-104](06-decisions.md#adr-104)'s principle that a claim which is not yet checkable is not yet false. **Asserts** exactly one `not-a-diagonal` violation naming the pair AD, and that both triangles are still fully placed: the claim is flagged, the figure is not destroyed. This is the row that forced the two-layer design; the issue's plan (*"mirror 3-D at apply time"*) does not cover it.
...
### `arc-equality-canonical-spelling-1000` — «קשת CD שווה לקשת DE» builds equal arcs: the spelling the operator ruled is the sentence (#1000, ADR-533)

**Guards against:** the remedy that returns the same refusal. A bare copula between two arcs («קשת CD היא קשת DE») reads as IDENTITY in Hebrew and is now refused, with the explicit form offered in the student's own labels — so the offered form has to build, end to end, or the teaching is a lie (#1156's failure mode). This scenario drives it through the real parse→fact→replay path and asserts **equal CHORDS on the built figure** rather than the command it lowered to. The REFUSAL half cannot live in this harness, which fails a scenario on any step that does not parse, by design; it is in `src/__tests__/issue-1000-arc-copula.test.ts`, where the refusal is asserted and the sentence it offers is driven back through the parser.

### `redundant-restatement-keeps-the-dof-count-1264` — «AB ⟂ BC» after «∠ABC = 90» removes no degree of freedom (#1264, ADR-536)

**Guards against:** the DOF accountant counting a dependent constraint row as a removed freedom. The operator's third line restates what the figure already shows at every seed, yet `freeDofCount` read 1 → 0 and the panel claimed «הכל נקבע» while «הציגו תצורה אחרת» went on producing six distinct shapes — a false claim about the student's own givens. The count is now raw − **rank of the constraint Jacobian at the solved figure** − the similarity gauge, so a row the others imply adds nothing; the ADR-424 vacuous-collinear special case became one member of this. This scenario locks the count through the real parse→fact→replay path, and the harness's `dofHonesty` oracle locks that the claimed freedom is samplable; the shape-still-varies half, the parallel-rows mechanism, the swapped-operand and mirror-order spellings, and the guard that a GENUINE second constraint still reduces the count live in `src/engine/__tests__/issue-1264-dof-rank.test.ts`.

### `contradictory-right-angles-refused-not-drawn-1328` — «∠ABC = 90» on an isosceles triangle with equal base angles is REFUSED, not drawn as a needle (#1328 P1, ADR-537)

**Guards against:** the tolerance drawing a figure that cannot exist. Both base angles right and a 0° apex: the givens hold only in the limit B = C, and the solver satisfied them within `ANGLE_EPS` (0.5°) with a 0.35° apex — a needle at flatness 6e-3, above the ADR-413 collapse floor and the ADR-513 notice band — drawn green with «נקבע במלואו». The accept gate now re-solves a thin declared polygon under a tightened degree tolerance; a needle cannot survive it, the step is refused naming the statement, and because a solution existed and was not a figure the refusal is a hard error, never a pending state. The prefix stays intact. The control pair, the legitimate thin triangles and the notice figures are in `src/engine/__tests__/issue-1328-tolerance-artefact.test.ts`.
...
### `angle-sum-contradiction-not-pending-1329` — «∠ABC = 100» · «∠ACB = 100» on a triangle is refused as IMPOSSIBLE, never pending (#1329, ADR-538)

**Guards against:** a contradiction the flex probe cannot tell from an under-determined solve being filed as an incomplete figure. Two angles of one triangle summing past 180° have no solution in any configuration; the step failed on its own and the classifier's "the residual moves" probe called it pending. The angle-sum prover beside ADR-417's metric prover proves the excess over the polygon's own (n − 2)·180° before the ladder runs, so the refusal names the angles and the bound and is a hard error. The unit cases — the 89° + 90° triangle that still builds, a quadrilateral's reflex angle that is still allowed, the diagonal and arc angles that are not summed, ADR-417's own cases — are in `src/engine/__tests__/angle-sum-feasibility.test.ts`.
...
### `bare-midsegment-binds-the-triangle-and-cycles-1368` — «קטע אמצעים» with nothing named binds the figure's triangle and cycles through three sides (#1368, ADR-545)

**Guards against:** the fully bare midsegment escalating and letting the LLM INVENT the side. ADR-199's cyclable mechanism anchors on a NAMED endpoint, so «קטע אמצעים» alone had no rule — it was `not-handled`, the model supplied the missing arguments, and the figure asserted a side the student never stated (ADR-052). Now it binds the figure's single triangle (0 or ≥2 REFUSES rather than picks) and emits a `midsegment-free` shape-variant whose three configurations are the three side-pairs — asserted here as: both endpoints are side MIDPOINTS on different sides, the segment is parallel to the third side, no `M`/`N` is minted (the endpoints are `@`-anonymous, ADR-297), and all three pairs are reachable by cycling. The «תוסיף» wrapper is a separate defect (#1358).

### `bound-with-its-value-on-the-boundary-samples-1351` — «BC>=10» then «BC=10» draws at every configuration (#1351, ADR-547)

**Guards against:** a bound's drawing aim out-pulling a given. The operator's own run (#1349): «משולש ABC» «BC>=10» «BC=10» «∠ABC = 90». ADR-529 made the boundary acceptable, but ADR-390's visible-gap aim still sat in the joint solver's cost, so at 15 seeds in 16 the solve converged to BC ≈ 10.2 and the given was refused — one configuration for «הציגו תצורה אחרת», and #1349's «כבר קיים» note lost with the pool. Asserted: every step holds at each of the first eight seeds, BC = 10 and ∠ABC = 90 at each, and at least three different triangles. The pool size, the angle twin, the in-band and derived pins, ADR-390's inside-draw and #1349's note through the real submit pipeline are in `src/app/__tests__/issue-1351-bound-aim-yields.test.ts`.

...
### `shared-endpoint-intersection-is-not-a-near-miss-944` — a crossing at the carriers' shared vertex passes the requirement gate (#944, ADR-489)

**Guards against:** found while verifying #942's play case (2026-09-08). «משולש ABC» + «D = חיתוך AB ו-BC» drew the **one correct figure** — the crossing of AB and BC *is* B, so D lands on B, the canvas writes the merged «B=D» (ADR-486) and a blue notice says they coincide — while the input panel simultaneously showed «לא נמצאה תצורה שמקיימת את כל הדרישות יחד … בדקו את הנתון האחרון שהוזן». Two surfaces, opposite claims, on the same screen. Measured clause by clause, exactly one conjunct of `meetsRequirements` failed: `intersectionsWithinSegments`, which demands a declared intersection sit strictly *inside* both carriers by `WITHIN_MARGIN`. Two segments sharing a vertex cross **only** at that vertex in every configuration, so no seed could ever satisfy it — not a tolerance set too tight, a requirement nothing can meet — and the auto-resolver's exhausted search left the amber fallback on a correct figure for good. Fixed by a **structural** exemption (`shareEndpoint`: do the carriers have a vertex in common?), never a slackened margin, since loosening `WITHIN_MARGIN` would re-admit the coincident-vertex near-collapse basin #569 exists to catch. Swept to all three sites that ask the question — the gate, the point-free sibling `segmentsCrossWithin` (ADR-383), and `reflectMaskForFailing`, which would otherwise keep blaming an exempt meet and aim the reflection search at a non-problem. **Asserts** the whole-pipeline verdict, because the defect was never in the drawing but in the gate that judged it: every step `ok`, no violations, D≡B surfaced as a coincidence rather than silently, and `intersectionsWithinSegments` agreeing with what is on screen. Unit locks in `src/__tests__/issue-944-shared-endpoint.test.ts` add the anti-vacuity check that the figure really carries the gated object, that an ordinary interior crossing is still refused at an impossible margin (so the predicate is proved to still do work), and that the shared-endpoint case passes at *any* margin — which is what makes the exemption structural rather than numeric.
...
### `bare-angle-reference-marks-the-corner-248` — the prod session's bare «זוית abc» marks the angle instead of escalating (#248, ADR-516)

**Guards against:** the one member of the angle family with no reading at all. Prod session `quvq3txq` (2026-07-20) typed «זוית abc» and met the scope brush-off; measured at HEAD, every bare spelling («זוית ABC», «∠ABC», "angle ABC", «זוית B», lowercase) was `not-handled` while every VALUED spelling built. ADR-516 makes the bare reference the THIRD reader of the `angleArms`/`angleValueOf` seam — no value ⇒ `mark-angle` — so the single-vertex and side-pair naming lanes are inherited, not copied. The scenario locks that the mark reaches the figure as an ARC (never a right-angle knee) at the vertex the student named, and that a VALUED angle stated in the same figure keeps its own meaning and still holds — the marker took nothing from it. DOF-neutrality, the query/refusal split, and the no-theft rows live in `src/__tests__/issue-248-bare-angle-marker.test.ts`.

...
## ADR-3D-072 — A solid is a BASE × a TOP: the quad-base registry, and a stated base noun is never silently swapped (issues #304 P1 bug, #305 feature)

**Status:** Accepted (2026-07-24). *Files: `src3d/engine/baseShapes.ts` (new), `src3d/engine/evaluate.ts`, `src3d/engine/apply.ts`, `src3d/engine/types.ts`, `src3d/parser/parse3.ts`, `src3d/parser/catalog3.ts`; `src3d/__tests__/quad-pyramid-bases.test.ts`.*

**Origin.** Operator, 2026-07-24: «in the 3d tool, i want to have `פירמידה שבסיסה מעוין` and it seems to draw something else.» It drew a **rectangle**-base pyramid: `פירמידה SABCD שבסיסה מעוין` lowered to `pyramid4gr`, and the `ישרה` form to `pyramid4r`. No error, no amber — every row read ✓.

**Class.** *A stated base-shape noun the rule cannot lower is silently replaced by the rule's UNSTATED-base default, asserting a shape the student never gave.* `rightPyramid` chose its base by a chain of positive tests (`square` / `equilateral` / `par` / `rect` / `tri`) ending in an unconditional `else` that produced the no-base-stated default — a free-aspect rectangle. So *recognising* a base noun and *being able to lower* one were conflated: any noun the chain did not happen to test took the "no noun was stated" path. This is an **M4 violation inside the parser** (a default surviving a contradicting statement) and the ADR-052 cardinal sin the LLM prompt is explicitly forbidden to commit ([ADR-3D-058](#adr-3d-058)) — committed here by the deterministic parser instead. It is also the docs/17 §2.2 shape: a proxy ("no noun I recognise") standing in for the semantic fact ("was a base stated?") — the fourth member of that family this week, after [ADR-3D-069](#adr-3d-069)/[070](#adr-3d-070)/[071](#adr-3d-071).

**Sibling audit (the grep test).** The class covered far more than the reported utterance. Silently → rectangle: `מעוין`, `מעויין`, `דלתון`, `טרפז`, `מרובע`, `מחומש`, `משושה` and En rhombus/kite/trapezoid/quadrilateral, in the labelled, bare and `ישרה` forms. Also found, unreported: a stated **triangle** base with 5 labels (`פירמידה ישרה SABCD שבסיסה משולש`) built a rectangle pyramid instead of refusing the contradiction; a stated **quad** base with 4 labels built a `tetra`; `פירמידה ישרה SABCD שבסיסה מקבילית` dropped the stated `ישרה` (it returned the free-apex `pyramidPar` regardless); and the same `else`-fallback in `rightPrism` made `מנסרה ישרה ABC שבסיסה טרפז` build a **triangular** prism.

**Root — the missing mechanism.** The deeper reason a base noun had no lowering is that **base × top was enumerated by hand**. `solidPositions` inlined the parallelogram ring three times (`prism4`, `parallelepiped`, `pyramidPar`), the rectangle three times, the square four; so a base written for one top simply did not exist for the other — the rhombus and the general quad had prisms and no pyramids. New `baseShapes.ts` defines each quad base ONCE (its free dims, its ring under the A=(0,0), B=(1,0) gauge, and its centre of symmetry), and a `QUAD_PYRAMIDS` table gives the whole 4-base pyramid family as (base, right?). **The five pre-existing kinds are driven by that same composition and come out bit-identical** — base dims first, then the top's, which is the order they were already written in — so the migration moved no figure (verified: the full 3-D suite green before any new kind was parsed). That equivalence is the point: the general path *subsumes* the special cases rather than sitting beside them ([ADR-3D-069](#adr-3d-069)'s lesson — the carve-out is what hides the gap). `prism4`/`prism4g`/`prism4sq`/`prism4r`/`parallelepiped` now take their rings from the same registry; their historical dim ranges are untouched.

**Decision (parser).** One shared `BASE_NOUNS` vocabulary answers "which base did the student state?" for every solid rule, and each rule's own kind table answers "can I lower it?" — `PRISM_BASE_KIND` and `PYRAMID_BASE_KIND`. A stated base with no entry, or one contradicting the label count, **DEFERS** (escalates → honest refusal) instead of falling through. The `parallelepiped` rule's `!/מעוין/` keyword bow-out (docs/17 tripwire 4) is gone — it now just asks whether the stated base *is* a parallelogram. The no-noun default is deliberately unchanged (a free-aspect rectangle, documented in `types.ts`).

**Decision (`ישרה`).** A right pyramid's apex sits over the base's **centre of symmetry**, so a right form exists only for a centro-symmetric base (square, rectangle, rhombus, parallelogram). `ישרה` over a kite, a trapezoid or a general quad has no defined apex and **defers** rather than inventing a centroid (ADR-052). Note this widens the project's previous "apex over the circumcentre" wording: over a rhombus or parallelogram the lateral edges come out equal in opposite *pairs*, which is the standard textbook figure; equal-all-four still requires a cyclic base.

**Gained.** `פירמידה שבסיסה מעוין` (rhombus, 1 free base DOF), `דלתון` (kite, 2), `טרפז` (trapezoid, 3), `מרובע` (general quad, 4) — each × free apex, plus the right form for the rhombus and the parallelogram; all bilingual, labelled / bare / apex-first. Every base's default drawing is held off its own special cases at *every* seed (a rhombus never renders square, a kite never a rhombus, a trapezoid never a parallelogram or right-angled) — the [ADR-3D-059](#adr-3d-059) discipline, generalised.

**Deferred (operator, explicitly).** Kite and trapezoid **prism** bases: no 572 corpus question uses one. They are now one table row plus one line each, which is the proof the mechanism is real.

**Locked by** `quad-pyramid-bases.test.ts` (21): the CLASS lock — for every noun × (labelled / bare / `ישרה`) × He/En, the kind actually built has the stated base or the rule defers, read from the engine's own table so it cannot be satisfied by hand-listing; the exact reported utterances; the contradiction and no-model defers; the `rightPrism` sibling; the pre-#305 forms unchanged; the geometry of each new base verified on coordinates at six seeds; the operator's verbatim sequence end-to-end (builds, base is a rhombus, composes with `נסמן:` and `|u| = 3`, and reshapes on "show another"); and a totality lock asserting `DIM_COUNT` / `VERTEX_COUNT` agree with the geometry for **every** `SolidKind` (the drift #288 found in the save whitelist, pre-empted here).

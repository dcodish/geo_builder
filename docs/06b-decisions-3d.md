...
**Fix:** `dataView` splits the gate into `translationPinned` (points block + planes block) and `orientationPinned`/`ownPairPinned` (the per-vector coords row); the query lane's `vectorForms` gets the same split (it had its own `hasFrame` copy). Locked by `frame-gates.test.ts` (6): the operator's exact sequence — no point coordinate prints, DE's own row still shows `(0, 2, 0)`, sibling vectors' coords null, the query lane agrees, and the positive direction (a real `D(0,0,0)` injection re-enables point printing). Full 3-D lane 1,223 green — every exam figure with real point pins prints exactly as before.

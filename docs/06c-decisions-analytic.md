...
## ADR-AG-133 — The fold DEFERS a constraint until its objects exist, and the LINE is its unit of application (#1242)

**Status:** accepted, 2026-09-21 · **Issue:** #1242 (bug, P2, `analytic`) · operator ruling 2026-09-19: *"the idea of order is not relevant since the diagram should either respect all input or refuse to build"*
**Requirements:** [02c](02c-requirements-analytic.md) R19, extended — entry-order independence for any consistent given set, and a refused line leaves nothing behind · **Design:** [04c](04c-design-analytic.md#the-fold-defers-and-the-line-is-its-unit-of-application-adr-ag-133)
**Ports** [ADR-104](06-decisions.md#adr-104) (2-D, 2026-06-23 — the same operator sentence) into the analytic fold; keeps [ADR-AG-015](#adr-ag-015)'s *a reference may not invent a point* and [ADR-AG-013](#adr-ag-013)'s *no topological sort*

**Measured before, on the operator's reversed order** «AD גובה לצלע BC» · «משולש ABC» (he reached it by editing an earlier line while playing T4 of the #1232 sheet):

```
faults      : unknown-reference B  — twice, once per constraint fact
outcomes    : ['faulted', 'created']
constraints : []                      <- both dropped
foot off BC : 1.288 × |BC|            <- a segment AD drawn, asserting nothing
```

`declare A`, `declare D` and `seg-AD` landed; the two pure constraints (`on-line-2pt`, `perpendicular`) were dropped, and the canvas showed a segment in a row that read as accepted apart from a small red `B`. Neither half of the ruling held: the input was not respected, and the tool did not decline to draw. And the residue was load-bearing — «M אמצע AD» typed after it read `created`, a midpoint of two points no line had placed.

**The class (docs/17 §1).** *The figure is built by a single forward pass that neither waits for information still to come nor undoes a statement it could not finish.* Not cevian-specific: «AB מקביל לציר x» before «משולש ABC» dropped its constraint the same way, invisibly, because that rule declares nothing before it constrains. The 2-D fold closed the first half of this class in June (ADR-104's deferral fixpoint) and the second with its atomic-group poisoning; analytic's `fold` was written later as one `forEach` and inherited neither.

**Mechanism — two fixpoints in `fold(facts, groupOf)`, in the order the plan gave them.**

1. **Deferral.** After the in-order pass, every still-failed fact that CREATES nothing — `constraint`, `selector`, `right-angle`, `area-of`, `tangent-of`, `on-kind` (the `NON_CREATING` set beside `fold`) — is retried against the completed construction until a pass lands nothing. The two constraints of the reversed order fail only because `B` and `C` do not exist YET; once the triangle declares them both retries land, and the figure is the working order's figure. ADR-104's limit comes with it: a creating fact is never deferred, since re-ordering it would strand its dependents — which is also why ADR-AG-013's invariant survives untouched: objects are still appended in declaration order, only a constraint may land later than it was typed. A fact is not retried against the very construction it last failed on (ADR-280's purity skip, cheap here but free).
2. **Atomicity.** `derive` hands `fold` each fact's line index. If any fact of a line still fails after the fixpoint, the fold re-runs without that line and every one of its facts carries the line's error, positionally — so `derive`'s rollup sees one refusal and no fragment reads as accepted. Removing a line can strand a later line that leaned on its partial objects (the midpoint above), so this too runs to a fixpoint. A clean list pays exactly one pass; a list with k faulted lines pays at most k+1, and `applyFact` has no solver in it.

`errors`, `effects` and `constraintFact` stay positional per fact — `derive`'s rollup and #1079's constraint blame read them unchanged. `derive` now reports a line's error ONCE (the same error on every fact of a faulted line is one refusal, not one per fact): measured before, the reversed order reported `unknown-reference B` twice for one sentence.

**The submit gate was audited, as the plan asked, and it is unchanged on purpose.** ADR-104's own amendment records that deferral in the replay layer was not enough in 2-D because the gate escalated a not-yet-satisfiable constraint. Here `decideSubmit` dry-runs the list with the new line appended and refuses on a fault at that index, so «AD גובה לצלע BC» typed on a canvas with NO `B` is still refused by name — that is ADR-AG-015 (a reference may not invent a point), and it is the "refuse to build" half of the ruling for a line that cannot be honoured *now*. The reversed order is reached by EDITING — deleting or rewording an earlier line — and that is where the fold now honours it. Locked in both directions: the fresh forward reference refused with `unknown-reference`; the same line on the triangle recorded; the reversed list deriving clean.

**Measured after.** Reversed order: `faults: []`, both lines `created`, foot off-line < 1e-6 and |cos| < 1e-6 at every seed 0–7, the same sorted constraint set as the forward order. Every permutation of «משולש ABC» · «AD גובה לצלע BC» · «AB מקביל לציר x» builds the same constraint set with the altitude holding. The unrescuable «AD גובה לצלע BC» alone: `faults` exactly one `unknown-reference`, `outcomes ['faulted']`, no object, no constraint, no point, no segment. «M אמצע AD» after it: `['faulted', 'faulted']`. The lock (`issue-1242-fold-deferral.test.ts`, 12) was run against the pre-change fold first: nine of its twelve cases red, the three "unchanged" cases green on both — so it holds the mechanism, not the number.
...
## ADR-AG-139 — A rule owns only what it parsed: the claim gate of ADR-AG-114 generalised to every claiming site, so a mis-owning rule never blocks the LLM escape (#1272)

**Status:** accepted, 2026-09-21 · **Issue:** #1272 (bug, P2, `analytic`) · operator, 2026-09-20 on #1271: *"we need to support all such forms of writing. I would expect the llm escape to assist in cases of minor deviations"* · round #1332
**Requirements:** [02c](02c-requirements-analytic.md) R104 (new) · **Design:** [04c](04c-design-analytic.md#the-parsers-rule-contract-adr-ag-017) — the rule contract

**Measured before (at 71fc71ad, re-measured at pickup and unchanged from the filing).** The LLM seam fires on exactly one code, `not-handled` (#1251) — the rest are OWNED answers and stand. Three sentences a rule matched by its NOUN and then refused a tail it never read: «פרבולה I: y^2=2x» → `bad-equation` about «I: y^2=2x», an equation the student never wrote; «נתון מעגל I - x^2+y^2=16» and «נתונה פרבולה I - y^2=2x» → the connective read as a SIGN, a hyperbola labelled "circle", vacant, `out-of-scope`. Each terminal, no escape. The audit the plan asked for found the same defect at every other claiming site — «שיפוע הישר AB הוא חיובי», «שטח המשולש ABC הוא גדול», «שיעור ה-x של A הוא שלילי», «האלכסון הראשי: משהו» all answered `bad-equation` about a Hebrew word — which is what makes this the class and not the reported instance. ADR-AG-114 had already ruled it for the curve nouns' Hebrew tails; the ruling never reached the value sites, and the curve exit itself did not see a name or a dash.

**The mechanism.** One helper, `claimable(tail)`, asked at every site that can answer `bad-equation` and at `matchCurve`'s single exit (where it absorbs ADR-AG-114's Hebrew test rather than sitting beside it). A tail is not the rule's when it carries Hebrew letters; opens with a connective dash — a hyphen followed by a space, or any en/em dash; or opens with a name the rule did not consume followed by a connective (`:`, a spaced dash, an en/em dash), the plane's own variables excepted so «y - 2x = 0» stays an equation. Then the rule answers `null`, the chain moves on, and the sentence ends at `not-handled` — the seam fires by itself. The seam was NOT widened: `reachesFallback(verdict)` (`app/submit.ts`, refused ∧ `not-handled`) is extracted so `App.tsx` and the lock ask the same function, and it is exactly what #1251 wrote. A hyphen glued to its term («-x+y=1», «-x^2-y^2=-16») is a sign and stays claimable — the one ambiguous spelling this leaves is «מעגל I -x^2+…», which reads as a sign.

**The `out-of-scope` audit.** `derive.ts` raises it only for a vacant object whose reason is a curve KIND the product does not draw (hyperbola, rotated, translated conic, not-a-curve) — a fact about an equation the tool DID parse, and «מובן, אך אינו נתמך» is the honest sentence for it. The dash cases never reach it now. A vacant circle («x^2+y^2=-9») is `does-not-exist`, as before. The wording stands.

**Not done, and why.** The dash is not admitted as a grammar connective («מעגל I - …» parsing directly). The operator's expectation names the escape for minor deviations, and admitting one more spelling per round is the drift ADR-AG-127 closed; if the logs show the dash spelling often, that is a feature with its own issue, not this fix.

**Measured after.** All nine class rows answer `not-handled` about the student's whole sentence and enter the seam; five owned refusals (a truncated equation, an unfinished number, a reserved coordinate, a degenerate role, a vacant circle) stay terminal; nine leading-minus / spaced-equation controls still record; the escaped circle comes back from the stubbed model as «נתון מעגל I: x^2+y^2=16» and records through the same gate.

**Consequences.** `parser/parseAnalytic.ts` (`claimable`; the diagonal, slope, area, component and curve-exit sites); `app/submit.ts` (`reachesFallback`); `App.tsx` (asks it). `__tests__/issue-1272-claim-gate.test.ts` (13). `docs/02c` R104; `docs/04c` the rule contract.
...
## ADR-AG-143 — A ring thinner than its tolerance is not a solution: the accept gate re-solves a thin ring under a tightened tolerance (#1334, P1; the ADR-537 port)

**Status:** accepted, 2026-09-21 · **Issue:** #1334 (bug, **P1**, `analytic`) · found validating round #1332's play sheet; operator: *"fix the p1"* · the analytic twin of [ADR-537](06-decisions.md#adr-537) (#1328, 2-D, ruled P1 the same day — *priority is the class, not the reach*)
**Requirements:** [02c](02c-requirements-analytic.md) R105 (new) · **Design:** [04c](04c-design-analytic.md#the-configuration-search-validity-then-preference-adr-ag-128) — the `whole` tier
**Sits beside** [ADR-AG-129](#adr-ag-129) (the ring collapse floor, `COLLAPSED_SIN_TOL`) without touching it; **extends** [ADR-AG-134](#adr-ag-134)'s "a non-solution is never a configuration"

**Measured before (2432c1b6).** «משולש ABC» · «AB = AC» · «∠ABC = 90»: `decideSubmit` → `record`. Per seed, the raw solve: 0 → |BC| = 0.0049 on 7.09-unit sides, no fault; 2 → 0.0084, no fault; 3 → 0.0045, no fault; 1, 4, 5 → unsatisfiable. The ring collapse test (|sin θ| < 1e-3 at a corner) caught seeds 0 and 3 in the raw figure, and the configuration search then walked to seed 2, where the needle's corner sits at 1.2e-3 — just above the floor — and showed it whole, every row green. The givens hold only in the limit B = C: for any small ε, C at distance ε from B along the perpendicular to BA gives |AC| − |AB| ≈ ε²/2|AB|, under `SATISFIED_EPS` (1e-6) for ε ≈ 0.004. The needle's thinness is the tolerance's, not the geometry's.

**The class.** A given set whose only solutions lie in a degenerate limit is accepted because the residual tolerance admits a nearly-degenerate figure, and the accept gate asks whether the residuals are small, never whether the tolerance is what made them small. Members: an isosceles base right angle (this); two right angles in one triangle (already refused, the solve does not converge); a stated apex of 0°.

**The mechanism — ADR-537's shape, ported.** In `evaluate`, after the residual check passes: the TRIGGER is a declared ring that is thin — `thinRingsOf` (`rings.ts`), min |sin θ| over its corners below `THIN_SIN_TOL = 5e-2` (2.9°; the 2-D `THIN_POLYGON_RATIO` band — an ordinary figure pays nothing). Then the same carrier system is re-solved from the converged point under `withToleranceFactor(TIGHT_TOLERANCE_FACTOR = 0.02)` — the factor lives in `solve.ts` beside `SOLVE_TOL`, scales the LM stop and the `ok` bar, and is always restored. A genuine thin triangle is an EXACT solution and keeps its shape; a needle bought with slack can only meet the tighter bar by collapsing past the floor, where `ringFaultsOf` calls it degenerate — and then the LAST constraint touching the ring (ADR-492's rule: the statement that completed the contradiction) is pushed to `unsatisfied`, so `whole` is false at that seed, the fold blames the student's own sentence, and `decideSubmit` refuses it. The re-solve is judged by the collapse predicate, never by `ok`: a re-solve that does not collapse changes nothing. Which constraint touches a ring is a structural walk (`mentionsAny`), not a per-kind list.

**Measured after.** Every seed 0–7 reports the needle unsatisfiable; seeds 0, 2, 3 blame «∠ABC = 90» and `decideSubmit` refuses it there naming the sentence. «משולש ABC» · «∠ABC = 90» builds; the solved 1.15° apex («AB = 10» · «BC = 0.2», inside the trigger band) and the pinned 1° apex build; two right angles still refused; #1273's figure untouched.

**The second half — the submit gate.** At the seeds where the raw solve already failed (1, 5) the solver's blame lands on «AB = AC» (line 1), and `decideSubmit`, which looked only for a fault on the NEW line, RECORDED the third line and showed the second red — the student's sentence accepted, an earlier true statement blamed. `decideSubmit` now also refuses a line whose dry run makes a fault APPEAR that the current figure did not have (compared by line and code): before the line the figure was whole, after it something is red, so that line is what turned it infeasible — ADR-492's "shortest infeasible prefix, blame its last fact", at the one chokepoint every typed line passes. The refusal names the new sentence.

**Sibling audit (docs/17 §1).** 2-D: ADR-537. 3-D: a solid's ring is a pivot solve with its own accept; not measured here, noted.

**Consequences.** `engine/solve.ts` (`withToleranceFactor`, `toleranceFactor`, `TIGHT_TOLERANCE_FACTOR`); `engine/rings.ts` (`THIN_SIN_TOL`, `minCornerSin`, `thinRingsOf`); `engine/evaluate.ts` (the gate, `mentionsAny`). `__tests__/issue-1334-tolerance-artefact.test.ts`. `docs/02c` R105; `docs/04c` the `whole` row.

## ADR-AG-144 — The solve vector holds every unknown: the 572 family — a parameter a given determines is SOLVED, a line may be created through a point with a FREE direction, a derivation restated about an existing point is a CONDITION, a numeral names a line, and the sign of a slope is a SELECTOR (#1317 · #1319 · #1320 · #1318 · #1298 · #1323)

**Status:** accepted, 2026-09-21 · **Issues:** #1317 (bug, P2), #1319 (feature, P2), #1320 (bug, P2), #1318 (bug, P2), #1298 (bug, P2), #1323 (feature, P2) — all `analytic` · **Operator rulings, 2026-09-21:** on #1317 *"All three, one piece of work. One design pass over the solver, then build all three, so a student can build the whole 572 figure"*; in the `/decisions` pass *"Grow the session to all six"*; on #1257/#1298 *a digit MAY name a line or a circle*. Not a round item — a named design session, one ADR, one PR.
**Requirements:** [02c](02c-requirements-analytic.md) R106–R110 (new) · **Design:** [04c](04c-design-analytic.md#the-solve-vector-holds-every-unknown-adr-ag-144) (new section), [the model](04c-design-analytic.md#the-model--objects-and-the-register-that-makes-them-free), [relations and the direction resolver](04c-design-analytic.md#relations-and-the-direction-resolver-adr-ag-024)
**Sits beside** [ADR-AG-005](#adr-ag-005) D7 (the three kinds of inequality — this adds the first SELECTOR over a derived quantity), [ADR-AG-047](#adr-ag-047) (a pin with two roots: the seed walks them, the panel lists both), [ADR-AG-118](#adr-ag-118) (a digit names a circle — the line twin), [ADR-AG-057](#adr-ag-057) (the through-a-point construction, which gains its free member), the #1046 ruling (a coordinate about an existing derived point — this is its converse). **Amends** [ADR-AG-009](#adr-ag-009) B2's unknown vector and [ADR-AG-126](#adr-ag-126)'s `knownCurve` sample.

### The figure

מבחן 20 עמ' 566 תרגיל 1: two lines `I`/`II` meet at `N`; a parametric line `(k+1)x+2y−12+5k=0` passes through `N` and cuts the y-axis at `M`; a second line through `M` cuts `I` at `A` and `II` at `B` so that `M` is the midpoint of `AB`; part (ג) puts a third line through `B` with a negative slope, meeting the x-axis at `C`, with area `ABC = 8.5`. Six sentences of it were unsayable, each measured on `main` @ `1d9a1892` (the "before" in each lock of `issue-1317-family.test.ts`), and all six fail on **one seam**: the solver's unknown vector held free VERTICES only.

### The seam — every unknown is in the vector, and the vertices go first

`evaluate.ts:579` `freeIds` was the whole unknown vector, two entries per free point; `sampleEnv` drew every parameter from the seed and handed it to evaluation as a constant. So a residual that depended on `k` was evaluated at whatever the seed drew and could not be driven to zero by any step the solver could take: `unsatisfiable` was the honest report of a solve never given the unknown. `src-analytic/CLAUDE.md` had said *"pinning a parameter is a numeric root-find"* since day one; it was simply not wired.

**`carrierSystem` now carries `syms` after `ids`** — every symbol of `paramRegister` — with `envAt(x)` writing the vector's parameter entries over the sampled environment, and every residual evaluated at `envAt(x)`. **The DOF report is rank over the joint vector** (`figureDofOf`): a parameter a given pins is subtracted exactly as a coordinate is, so `reportedDof` no longer adds the register's length (a pinned `k` used to count as a freedom the figure did not have). **A declared domain filters a pin's roots silently** (D7 kind 1): `solveMultiStart` takes an `accept` predicate, and a converged solve that drove a parameter out of its domain is a start that did not converge. **Parameter restarts:** only when nothing converges, the parameters are re-sampled at other seeds, so a root the first sample cannot reach stays reachable; a two-root pin (`a² = 4`) is reached by different configurations and the seed walks them — the ADR-AG-047 branch question, answered by the seed exactly as a crossing's is.

**Two stages, and the second is the decision.** Measured on «A(0,0)» · «B(8a,0)» · «נקודה M» · «MA = MB»: solved jointly from the seed, the descent drove `a` to 0 — B onto A, where |MA| = |MB| holds for every M — and drew the collapsed figure as the configuration. A parameter in the vector is a knob, and a least-squares descent reaches for every knob it has. So the solve runs first over the free vertices with every parameter held at its sample (the solve as it always was), and a figure that converges there keeps its sampled parameters — an unpinned parameter stays the seed's and still moves on «הציגו תצורה אחרת» (ADR-052 both ways). Only when that stage does NOT converge do the parameters join the vector, from the same starts: that is the case the old solve could never reach, and the only case in which moving a parameter is honest. The locus tracer asks for `params: 'fixed'` — a locus is a named point's freedom at one configuration of the parameters, and whether a parameterised locus sweeps its parameter is #1186's question, left where it was.

**The solve as built — five findings measured on the exam's own figure, none of them guessable from the code.** (1) *The membership residual was an absolute value* (`curves.ts`): the Jacobian is numerical, and within a step of the line `|g|` differentiates to ~0, so every incidence row vanished from the Jacobian exactly as the iterate approached the solution — a linear figure lands in one Gauss–Newton step and never showed it; a free direction couples the incidences nonlinearly and the descent stalled just above tolerance at half the seeds, with the right geometry in hand. The residual is SIGNED now (`isOn` takes the magnitude). (2) *A joint descent over a dozen unknowns from a compromised start goes to a bad basin at a third of the seeds* — the second free line spun to the far root, the sign filter rejected it, and every restart landed in the valley where C sits far out and the area is nearly met. So stage two walks the constraints IN THE STUDENT'S ORDER from stage one's effort, each solve warm-started from the last (the 2-D fold's replay, one given at a time), polishes against the whole system, and repeats the walk from the parameters re-sampled at other seeds; the joint multi-start is the fallback. A retry starts its vertices from the seed — a retry that kept the effort's far-flung crossing turned the free line parallel to that crossing's line, the pole, every time. (3) *A wrong-sign root is one root away, not one figure away*: the two roots of part (ג) are slopes −4 and +0.12 and the basin boundary falls inside the “negative” half, so re-sampling everything threw the figure back to the seed while the same root came up again; a converged wrong-sign solution is re-seeded at spread angles inside its half and the walk resumes from the converged figure. (4) *The sign sentence names the LINE*: «שיפוע הישר l5 שלילי» resolves to the curve `line-l5`, never to the free angle, so seeding and root selection keyed on `Direction.free` never fired on the exam's sentence and the sign held at 22 of 24 seeds by luck — `freeAngleOf` reaches the angle through the name (a `line-at` with a free direction), in the seeding, the admissibility and the re-seed alike. (5) *Cost*: a descent parked at a saddle spent the whole budget shaving a millionth per step (`solveLM` exits after eight such steps); stage one asks its question from the seeded start alone when parameters exist, with a 40-iteration budget; `evaluate` is memoised per (construction, seed) so the drawable walk and the knowledge gates share their work; and a determined figure that is whole but narrow walks 8 seeds for a prettier root, not 24 — with no freedom left another seed can reach only another root. Measured: part (ב) and part (ג) whole at 24 of 24 seeds; the 15-line derivation 4.5 s → 1.0 s, a mid-figure line ~80 ms.

**Amendment, 2026-09-21 (#1343, found at play, P1).** The parameter row printed `m = -3.46` for «m<0» — a sampled value as knowledge. `figureSignature`, the one definition of “the same configuration”, signed points and curves only; a parameter nothing drew left every seed signing alike, `distinctConfigSeeds` answered one seed, and one sample read as certainty — #1282's shape, for the environment. Two chokepoints, not the row: the signature now signs the parameters the figure USES (`Figure.usedSymbols`; a declared symbol nothing reads is part of no configuration a student can see, so «הציגו תצורה אחרת» stays honest about it, and a free direction never signs — θ and θ + π are one line); and “used” is `usedSymbols(c)` — objects' `symbolDeps` ∪ every `sym` node of every constraint, structurally — so a `k` an area given pins is used and pinned, while a symbol nothing reads is never asked of the gate and prints its domain with «(לא בשימוש בשרטוט)». Found by its own lock: a symbol used ONLY by a constraint («שטח המשולש ABC הוא k») was never in the register at all — objects fed it, constraints did not — so `k` was never sampled, evaluated to NaN and the area given was silently unjudged (#1014's defect one layer over). `paramRegister` now reads `constraintSymbols` too (length expressions' private-use term placeholders excluded), and the solve vector pins it: the area gives k = 9. Lock: `issue-1343-unused-parameter.test.ts`.

**Measured after.** «k הוא פרמטר» · «נתון הישר l3: (k+1)x+2y-12+5k=0» · «N(-2,4)» · «N על הישר l3» → `k = 2` at every seed, no fault, `l3` prints `3x + 2y − 2 = 0`, the panel's parameter row reads `k = 2` (through `isKnowledge`, the same gate every number passes), freedom 0. The slope form pins `k` the same way; «A(a,0)» · «B(0,0)» · «AB = 2» reaches both roots across seeds and only `a = 2` under «a > 0». The crossing form of the exam builds with no fault and `N` on all three lines; a genuine contradiction («שיפוע הישר l3 הוא 7» after the pin) is refused by the submit gate naming that sentence (ADR-AG-143's second half), and a fault is reported once per line.

### The five members on the seam

- **#1319 — «דרך N עובר ישר», «דרך M עובר ישר l4».** A `line-at` whose `dir` is `{ k: 'free', sym }` — a DIRECTION PARAMETER in the register: `symbolDeps` reports it (the #1014 rule, applied to a symbol no equation holds), so it is sampled like any unstated magnitude and solved like any parameter once a later given pins it. The angle is unbounded (a direction is periodic in π), which keeps the domain filter out of it. The emitted line is NORMALISED (`normalizedLine`): θ and θ + π are one line and the raw triple flipped sign with them, which read to the knowledge gate as a line that moves when it does not. The symbol is the tool's (`θ_<id>`, `directionSymbol`), counted in the cue and never a parameter row — like a free vertex's coordinates; the line's own row shows its equation once the direction is knowledge. A named free line resolves everywhere a name does: `curveByName` (types.ts) is the ONE by-name lookup, replacing three inline copies that matched `kind === 'curve'` alone, and `curveAtOf` gains a `line-at` arm (`lineAtCurve`, shared with the object walk), so «A נקודת החיתוך של הישר l4 עם הישר 1» is an incidence the solve can measure. `carrierOf` still answers `null`: the freedom lives in the register.
- **#1320 — «M אמצע AB» about an M that exists.** A `derived-at` constraint at the M1 boundary — for EVERY `DerivedRule`, never a per-rule list — whose residual is the rule's own closed form (`evalRule`), two components scaled to the parents' spread. #1046 built the coordinate direction and left this one a name clash on the reading that "M is the centroid" after M exists is a second definition; the exam says otherwise — M is the y-axis crossing AND the midpoint, and their equality is what fixes the second line — so the asymmetry is removed. An exact restatement is still absorbed as `known` (#1045). Four locks that asserted the clash are inverted, each naming this ADR; the invariant they protected (one name, one object, one drawn M) is asserted unchanged. A false restatement about placed points is `unsatisfiable`, naming the sentence.
- **#1318 / #1298 — «הישר 1», «הישר I».** `LINE_NAME` gains a numeral alternative (`I|II|III|IV|V|[1-9]`, the circle's range for the circle's reason) with the SEPARATOR LOOKAHEAD that is the token's whole safety (`«הישר 2x-y+8=0»` opens with a coefficient) — the [IVX] discipline of ADR-AG-006, fourth instance. The numeral alternative precedes the two-point run, so `II` is a numeral and never *"through I and I"* (the phantom point); a two-point name that repeats its letter is refused `repeated-vertex`. The noun-less forms («משוואת AB היא», «AB:») keep the plain token: with the noun dropped a Roman numeral is a CIRCLE (#1072). The panel row follows the #1216 circle precedent («מעגל 1»): a numeral-named line is called «ישר 1» / «line 1», id `line-1`, and every by-name lookup matches the id as well as the name, so the student's own token resolves. In an operand slot a numeral needs its noun («הישר 3», never a bare «3» — a digit in free text is a number before it is a name).
- **#1323 — «שיפוע הישר l5 שלילי».** A `sign` SELECTOR over a `Quantity` (a discriminated union — `slope` today; a length, an area or a coordinate would be further members with their own reader, never a fourth value keyword in the slope rule, which would produce the #1201 shape). Judged inside validity (`selectorsHold`), so a configuration with the wrong sign is not one the tool draws — `drawableAt` walks past it and a figure with no freedom left is reported on the sentence (the #1071 predicate `derive` already applies). **#818 (3-D) confirmed as the failure mode, and answered by SEEDING:** a sign about a FREE direction folds the sign into the sampled angle before the solve starts, so the descent begins in the basin the sentence names; the post-hoc check keeps the last word. Measured on the exam's part (ג): the area pins the direction to two roots (C at 4.5 or −12.5), the sign picks one, and every seed honours it. The rule runs BEFORE the slope rule: «the slope of l1 is negative» was accepted as `n·e·g·a·t·i·v·e` (the #1321 letter-run trap on this sentence). **«m<0» stays what it is** — F11's bare-inequality declaration, the corpus's own declare-before-use, which cannot be refused without refusing «0<k<6» before its parabola; what changes is that it is not silent: the panel row for a declared symbol nothing uses says so (`paramUnused`), and the sentence that means a slope sign now exists and is in the catalog (the LLM's vocabulary).

### Sibling audit (docs/17 §1)

2-D: parameters are not a 2-D concept (a magnitude is a segment or an angle); the free-direction line is 2-D's free line through a point (ADR-052's DOF classes). 3-D: «ישר k» is a free line (#552); no parametric curves. The class is analytic-only.

### Consequences

`engine/types.ts` (`Selector.sign`, `Quantity`, `line-at.name`, `curveByName`); `engine/solve.ts` (`Direction.free`, `Constraint.derived-at`, `dirVector(…, env)`, `solveMultiStart(…, accept)`, `freeDirectionSymbol`); `engine/carriers.ts` (`symbolDeps` for a free direction, `reportedDof` = the joint freedom, `directionSymbol`/`isDirectionSymbol`); `engine/evaluate.ts` (`carrierSystem` with `syms`/`envAt`/`params`, the two-stage solve, `figureDofOf`, `lineAtCurve`, the sign arm of `selectorsHold`, sign seeding, `knownCurve` sampling the drawable figure); `engine/apply.ts` (the `derived-at` lowering, the sign selector's references, `line-at.name`); `engine/locus.ts` (`params: 'fixed'`); `engine/curves.ts` (the signed residual); `engine/solve.ts` (the stall exit); `engine/evaluate.ts` (`evaluateMemo`, `freeAngleOf`, the student-order walk, the determined-figure walk budget); `parser/parseAnalytic.ts` (`LINE_NUMERAL`, `LINE_NAME`, `LINE_NAME_PLAIN`, `lineNameOf`, `NAMED_LINE`, `THROUGH_FREE_*`, `SLOPE_SIGN_*`); `parser/catalogAnalytic.ts` (nine entries); `app/ask.ts`, `App.tsx` (parameter rows through the gate, `paramUnused`), `i18n`. Locks: `__tests__/issue-1317-family.test.ts` (62), four inverted locks in `derived`, `lowering`, `parser` and `issue-1272-claim-gate`. Not built, deliberately: #1171 (a free line with NO anchor, 2 DOF — the register now has the slot for its direction; its offset still needs a home), #1186 (a locus that sweeps its parameter), #1324 and #1322 (planned behind this pass, not on the seam), a prefix search in `derive` that would blame only the last line of a contradiction on a LOADED file (the submit gate does it per typed line).

## ADR-AG-145 — A letter run is not automatically a product of parameters: the word test moves to the tokenizer every value slot passes through (#1321)

**Status:** accepted, 2026-09-21 · **Issue:** [#1321](https://github.com/dcodish/geo_builder/issues/1321) (bug, `P2`, `analytic`) · round [#1345](https://github.com/dcodish/geo_builder/issues/1345) · found answering the operator's question about adding `m = tan α` to a line's properties ([#1322](https://github.com/dcodish/geo_builder/issues/1322)) — before that feature can exist, a student who types `tan` must not be silently given a different figure
**Requirements:** [02c](02c-requirements-analytic.md) R111 (new) · **Design:** [04c](04c-design-analytic.md#a-letter-run-is-not-automatically-a-product-adr-ag-145) (new section)
**Fifth instance of** [ADR-AG-006](#adr-ag-006)'s `[IVX]` discipline — *a discriminator measured only against the inputs it was designed for has not been measured*; **keeps [#1068](https://github.com/dcodish/geo_builder/issues/1068)'s ruled word boundary byte-identical**

**Measured before (`043c2db3`).** «נתון הישר l1: y=2x» · «שיפוע הישר l1 הוא tan(30)» → `created, created`, **no fault**. The slope of `y=2x` is 2 and `tan 30° = 0.577`, so a correct tool must refuse. It did not, because the constraint it built was not about a tangent: the value lowered to `t · a · n · 30` — three symbols the student never wrote. The control is what makes it a defect and not a gap: «שיפוע הישר l1 הוא sqrt(3)» is `unsatisfiable`, correctly, because `sqrt` is a real token that gets MEASURED.

**The class.** *An unknown letter run in a VALUE SLOT is accepted as a product of new free symbols.* Juxtaposition is why this expression parser is hand-written — `2a`, `2ax`, `25k²`, `4√5` are products in the exam's notation — and `√` is the only function in the language (`sqrt` normalises to it). Every other function name is a run of letters, and a run of letters was a product. The register takes its parameters from what expressions USE ([#1014](https://github.com/dcodish/geo_builder/issues/1014)), so each phantom symbol became a free DOF and the residual could always be driven to zero: **the false given built green**. Members: `tan`, `sin`, `cos`, `log`, `ln`, `arctan` — with or without parentheses — and any future function name. [ADR-AG-144](#adr-ag-144) already recorded one member in passing («the slope of l1 is negative» read as `n·e·g·a·t·i·v·e`) and fixed that SENTENCE; this fixes the slot.

**The word test is not invented here — it is MOVED.** `parseAnalytic`'s `HAS_A_WORD` already answers *"is this letter run a word?"*, and [#1068](https://github.com/dcodish/geo_builder/issues/1068) ruled its boundary: **SPACE-DELIMITED and ≥3 letters**, deliberately, so that `x²+y²-2abc=0` stays a legal product of juxtaposed parameters. That decision stands and its lock is untouched. The defect is that the test guarded **one branch** — the bare-equation branch at the bottom of `parseLine` — and a value slot inside a *recognised* sentence never passed through it. It now lives in `expr.ts`'s tokenizer, the one place every value slot in the grammar routes through, so one change makes every sentence honest at once (docs/17 — the chokepoint, never the calling rule).

**One boundary is added, for the form a space cannot delimit.** `tan(30)` is not space-delimited, so the ruled word test cannot see it. A run of **≥2 letters immediately followed by `(`** is a function APPLICATION, and `√` is the only function there is. A single letter keeps juxtaposition, because `k(x+1)` is a product and the corpus writes it. **Never a list of function names** — a list catches `tan` and misses the next one, which is exactly what the `[IVX]` lesson is about.

**A refusal, not a silent reinterpretation.** The line comes back `bad-equation` naming the student's own text («tan(30)»), which reaches the LLM seam the way any unhandled sentence does. Dropping a stated given is the honesty line; refusing loudly is not crossing it.

**The second half — found BY this fix, in the catalog's own English row.** With the tokenizer honest, catalog F19's English entry «the distance between A and B = 10» stopped parsing. Measured on pristine `main`, it had been lowering to `t · h · e · |AB| = 10` — the measure noun `DISTANCE` spelled Hebrew's article (`ה?מרחק`) but not English's, so `the ` was left unconsumed, reached `parseExpr`, and became three phantom symbols multiplying the real distance term. **Satisfiable at any length: the English row never pinned the distance.** The catalog lock asserts each row PARSES, so it was green the whole time — only a tokenizer that refuses words could surface it. `NOUN` beside it already spells the article for `line`; the measure noun now does too, and the two languages lower to the identical length expression.

**Measured after.** «tan(30)», «tan 30», «sin(30)», «cos 45», «log(100)», «arctan(2)», «ln(5)» in a slope slot are all refused naming the sentence; «sqrt(3)» still measures and refuses on the arithmetic; «שיפוע הישר l1 הוא 2» still builds. `2a`, `2ax`, `25k^2`, `4√5`, `ab`, `k+1`, `k(x+1)`, `(k+1)x`, `2π` and #1068's own `2abc` all still parse. Both F19 spellings now pin |AB| = 10 exactly. Analytic lane: 156 files / 2273 tests green.

**Sibling audit (docs/17 §1).** 2-D and 3-D do not have a symbolic expression layer — a magnitude there is a segment, an angle or a number, and there is no juxtaposition rule to absorb a letter run. The class is analytic-only.

**Consequences.** `engine/expr.ts` (`LATIN_RE`, the letter-run rules in `tokenize`); `engine/lengths.ts` (`DISTANCE` carries the English article). Lock: `__tests__/issue-1321-letter-run.test.ts` (8). `docs/02c` R111; `docs/04c` the new section.

## ADR-AG-146 — A length token is a WHOLE name, never a pair out of the middle of a word: the English «=» angle form stops being a length (#1333)

**Status:** accepted, 2026-09-21 · **Issue:** [#1333](https://github.com/dcodish/geo_builder/issues/1333) (bug, `P3`, `analytic`) · round [#1345](https://github.com/dcodish/geo_builder/issues/1345) · found while fixing [#1330](https://github.com/dcodish/geo_builder/issues/1330) and filed rather than fixed there
**Requirements:** [02c](02c-requirements-analytic.md) R60 (the right angle's spellings — extended to the English `=` form and to case) · **Design:** [04c](04c-design-analytic.md#the-parsers-rule-contract-adr-ag-017) — the rule contract
**Completes** [ADR-AG-145](#adr-ag-145) (#1321) for the case a letter run does not survive; **closes** the [#1151](https://github.com/dcodish/geo_builder/issues/1151) honesty failure at its source instead of ahead of it

**Measured before (`043c2db3`, re-measured at pickup).** «angle ABC is right», «the angle ABC is 90» and «∠ABC = 90» all lower to a perpendicular. «angle ABC = 90» lowers to a **`length-eq`** whose left side is `a·n·g·l·e·|AB|·C`. On «משולש ABC» · «AB = AC» the submit gate answers `record` — the issue's body says `already-follows`, which is the divergence ADR-W-064 expects: [ADR-AG-143](#adr-ag-143)'s gate landed in between. The verdict differs; the defect does not. The Hebrew twin «זווית ABC = 90» was never affected, because the Hebrew length grammar does not admit «זווית».

**The class, one layer below where the issue put it.** The issue named rule ORDER — `LENGTH_EQ` running before the angle rule. Measured, order is not what fires: the rule's own guard is *"at least ONE side must mention a LENGTH"*, and it is **satisfied**, because `LENGTH_TOKEN` found `AB` **inside the word «angle ABC»**. The real class is *a length token taken out of the middle of a word*, and this file's own docblock has recorded where it leads since [#1151](https://github.com/dcodish/geo_builder/issues/1151): *"the token missed, `LENGTH_TOKEN` then ate `AB` out of the sentence, and the student was answered about a DIFFERENT measurement."* Each time it bit, a frame was moved AHEAD of the token — areas first, then point-to-line distances — but **the token itself was never bounded**, so every word no frame claimed was still fair game. «ANGLE ABC = 90» is the extreme: three measurements (`AN`, `GL`, `AB`) out of one word.

**The mechanism.** `LENGTH_TOKEN` gains letter boundaries — `(?<![A-Za-z]) … (?![A-Za-z])`. Letters only, not digits: `2AB` is a coefficient times a length, the juxtaposition the expression layer exists for, and the token's own optional digits (`A1B2`) sit inside the match. A run of three or more capitals (`ABC`) now yields nothing here, which is correct — it is a polygon, and the area frame that owns it already runs first. No rule ordering changed, and no noun list was added: a list of nouns the length grammar "does not own" would have to be extended for every word anyone ever types, which is the patch tripwire docs/17 names.

**Why this is the same root as [ADR-AG-145](#adr-ag-145), and why both are needed.** #1321's tokenizer already refuses a space-delimited word, so with it alone «angle ABC = 90» is correct — the leftover `angle` never reaches a symbol. It does **not** reach «ANGLE ABC = 90», where `LENGTH_TOKEN` consumes `AN` and `GL` first and no ≥3 letter run is left to refuse. One fix stops leftovers becoming parameters; this one stops the token creating the leftovers. Together the sentence means the same thing in every case.

**Measured after.** «angle ABC = 90», «Angle ABC = 90», «ANGLE ABC = 90», «the angle ABC = 90», «the angle ABC is 90» all lower to exactly what «angle ABC is right» lowers to. «AB = 4», «AB = BC», «AB + BC = 10», «2AB = BC», «A1B2 = 5» are unchanged lengths; «שטח המשולש ABC הוא 24» still declares its polygon and its area. `parseLengthExpr('ANGLE ABC')`, `('angle ABC')` and `('ABC')` are all `null`. Analytic lane: 156 files / 2273 tests green.

**Scope, measured and unchanged.** A non-right angle value falls through in BOTH languages — «angle ABC = 45», «זווית ABC = 45» and «ANGLE ABC = 45» are all `not-handled`. There is no language disparity to fix here; an angle residual other than 90° is a capability this tree does not have, exactly as [ADR-AG-142](#adr-ag-142)'s own lock states.

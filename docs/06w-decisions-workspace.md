...
## ADR-W-073 — The ruling vocabulary is widened from the live corpus, and the passes write ONE canonical heading (#1325; amends ADR-W-049)

**Status:** accepted, 2026-09-21 · **Issue:** #1325 (debt, P3, `workspace`) · round #1332
**Requirements:** none (internal) · **Design:** [22](22-workflow.md) §2b — the decisions pass and the escalation exit write the canonical heading; the skills carry the line
**Amends** [ADR-W-049](#adr-w-049) — the guard's marker list and the write side

**Measured.** `node scripts/queue-hygiene.mjs` on 2026-09-21 reported #1164 and #1184 as `unlabelled-question`. Both were ruled: #1164 by «**Operator ruling, 2026-09-19:** *"agree. it should refuse."*» (a comma after the phrase, which the second marker's separator class did not admit) and #1184 by «**Arm 2 is unblocked — #1188's question 2 is ruled** (operator, 2026-09-20)» (no "operator ruling" phrase at all). The status report carried both into "Waiting on you"; the operator asked what was needed; the answer was nothing — the decision-slot cost ADR-W-049 exists to prevent.

**The class.** A closed vocabulary against passes that each invent their own phrasing: the false positive recurs with every new spelling. Two moves, both needed. **Widen from the corpus:** the separator after «operator ruling/decision/answer» admits a comma; the verb form «is ruled» counts when the operator is named in its trailing parenthesis. Whitespace alone is deliberately not a separator and the bare verb is deliberately not enough — "needs an operator ruling on X" and "until it is ruled" are a session ASKING, and reading them as answers would clear a label nobody answered (ADR-W-049's rule that only the operator's word counts, kept). **Canonicalise the write side:** the `/decisions` skill's transcription and the resolution of a round's escalation open with `## Operator ruling — YYYY-MM-DD`, verbatim — `CANONICAL_RULING_HEADING` in the script is its one definition, and the first marker is its shape. Widening alone re-opens next month; the canonical line is what closes it.
...
## ADR-W-075 — One LLM request harness, in `server/`: the product supplies prompt DATA and the proxy composes (#1359)

**Status:** accepted, 2026-09-22 · **Issue:** [#1359](https://github.com/dcodish/geo_builder/issues/1359) (debt, `P2`, `workspace`) · operator, 2026-09-22: *"I think its of value to have all work the same way for future maintenance and user standardization"*, and on the LLM specifically: *"we want the llm to work in the same mechanism as it is in 2d and 3d where it works well."*
**Requirements:** none (internal) · **Design:** [04s](04s-design-server.md#the-one-llm-request-harness-adr-w-075) (new section)

**Measured before.** The lane was written three times with **zero shared code**: `src/parser/llmShared.ts` (178 lines), `src3d/parser/llmShared3.ts` (119), `src-analytic/parser/llmSharedAnalytic.ts` (130), plus three clients (242 / 25 / 73) — 767 lines, `shell/` imports: none. All three declared the **same model** (`claude-haiku-4-5`), the **same** 1024-token budget and the **same** `emit_steps` tool name independently, and composed the same four-section prompt skeleton with the same message assembly.

**Decision.** The harness — model, token budget, tool name, tool schema, the intro→rules→vocabulary→examples skeleton, the request body, and the response reader — lives once in **`server/llm/harness.ts`**. A product exports a `PromptSpec`: its intro (as a function of the tool name, so it never hardcodes `emit_steps`), its rule lines, its vocabulary renderer, its examples, and its two tool descriptions. `server/parseHandler.ts` composes on dispatch.

**Why `server/` and not `shell/`, which is where the sharing instinct points.** `BOUNDARIES.json` forbids **`server → shell`** — *"the proxy has no UI; nothing in browser chrome belongs in it"* — and the proxy is what builds these requests. The complementary edge **`src → server`** is forbidden too, so a product cannot import the harness either. Those two rules together force the shape: the product exports **data**, and composition happens on the `server → product` edge the registry already sanctions. The spec is structurally typed where it is consumed, so a product whose shape drifts is a compile error in `parseHandler`.

**The other half of this lane is NOT here, and cannot be.** The sequence gate and the honesty-gate battery run on the steps *after* they come back, client-side, and belong in `shell/`, which every product may import ([#1355](https://github.com/dcodish/geo_builder/issues/1355), [#1356](https://github.com/dcodish/geo_builder/issues/1356)). Two halves, two homes, one per allowed edge.

**NOT ONE CHARACTER of any prompt changed, and this was verified rather than intended.** Each product keeps its rules text verbatim. Harmonising the wording is a separate decision with real risk: it changes model behaviour in two tools the operator has said work well, and no test can catch a prompt regression. The refactor was gated on a byte-identity check of all three request bodies against goldens captured before the first edit — and it **failed twice**, which is the whole argument for capturing them:

1. **The intro line-breaks differ.** 2-D wraps at *"…into an ordered" / "list of…"*; 3-D and analytic wrap at *"…into" / "an ordered list of…"*. Composing the intro from a domain noun silently rewrote 2-D's prompt. The intro is therefore product-supplied.
2. **The tool DESCRIPTIONS differ** — *"Return the geometry construction…"* vs *"…the 3-D space/vectors construction…"* vs *"…the analytic-geometry construction…"*, and 2-D alone appends *"(each one a supported phrasing)"* to the array description. These strings go to the model. They stay product-supplied, and are now asserted so the difference is a choice rather than an accident.

**Two things the extraction surfaced and fixed.**

- **An undeclared boundary edge.** `server → src-analytic` has existed since [#1251](https://github.com/dcodish/geo_builder/issues/1251) wired analytic to the proxy and was never in `BOUNDARIES.json` — neither allowed nor forbidden, so the isolation guard never saw it. Registered.
- **A source-scanning lock, broken exactly as docs/28 §5c predicts.** `parse-tool-registry.test.ts` read `parseHandler.ts` **as text** and grepped for `PROMPT_BUILDERS[tool]` and for `buildLlmRequest\w*`. The extraction changed no behaviour and turned it red. It is rewritten to call the decision: the distinctness check now compares the **composed prompts** (two different builder names can still render the same prompt — the names were never what mattered), the unknown-tool refusal and its ordering against the daily quota are exercised through the real `handleParse`, and each product's prompt is asserted to carry its own vocabulary and not a sibling's.

**The cross-product lock (docs/28 §5c).** `server/__tests__/issue-1359-prompt-lane.test.ts` runs one row set over **every** registered spec: the shared model/budget/tool, the message shape, the four sections **in order** (asserted by position, so a moved section fails rather than passing because the words are still somewhere), every example rendered, and — the part a per-tree test could never do — that each product states the **ADR-052 never-invent rule**, the **never-reorder-a-letter-run rule** (#536/#555) and the answer-in-the-student's-language rule. A product that quietly drops one of those starts inventing givens, and no sibling's test would have noticed. A new product joins by appearing in the registry; every row then applies to it with no edit.

**Consequences.** `server/llm/harness.ts` (new); `server/parseHandler.ts` (`PROMPT_SPECS`, composes via `buildRequest`); `src/parser/llmShared.ts`, `src3d/parser/llmShared3.ts`, `src-analytic/parser/llmSharedAnalytic.ts` (now prompt DATA; 2-D keeps `figureContext`, its one client-side export); `BOUNDARIES.json` (`server/llm` classified, `server → src-analytic` declared). Locks: `server/__tests__/issue-1359-prompt-lane.test.ts` (new), `parse-tool-registry.test.ts` (rewritten behavioural), and the per-tree contract tests repointed at their spec's own sections.
...
**Analytic had no gate at all** — whatever the model wrote was what got parsed. Its live case is on [#1297](https://github.com/dcodish/geo_builder/issues/1297): the model renamed the student's «ישר 1» to `l1`, and the student was then refused for referring to their own line by the name they gave it.

**Decision.** The algorithm moves to **`shell/llm/sequenceGate.ts`** and all three products call it. Measured before: the two shipped copies were the same code line for line, differing in exactly three things — the label pattern, the utterance normaliser, and a length-preserving transform applied to the line before matching. Those three are now a `SequenceGateSpec` the product passes in. Analytic's would have been the **third** copy; it is instead the seam's first new consumer, which is the cheapest possible proof the extraction is real.

**A difference deliberately PRESERVED rather than unified.** 2-D masks the ADR-118 area marker `S` for matching but emits from the ORIGINAL line, so a line it does not rewrite returns byte-for-byte; 3-D canonicalises prime marks and emits from the canonicalised text. Unifying those would have changed output in a shipped tool, so `prepareLine` returns `{ match, emit }` and each product keeps its own behaviour.

**Where it runs in analytic.** In `runFallback`, **before** `decideSubmit` — the restored spelling is what must be parsed, recorded and read back; restoring afterwards would fix the text and leave the figure wrong. The corrections ride out on the outcome as `restored` and are logged on the submit event, as both siblings do: without that field a submit that was silently corrected is indistinguishable from one that needed no correction, which is how #536 stayed invisible.

**The cross-product lock (docs/28 §5c), including the half that is usually skipped.** The rows live once in `shell/__tests__/fixtures/sequence-gate-rows.ts` as a pure `sequenceGateFaults(gate, opts) → string[]`; each tree has a thin lock calling it; 3-D's primed alphabet is an **explicit option**, never a silent exclusion. And a **meta-lock** in `shell/` runs the same rows against six deliberately broken gates — one that does nothing, one that restores without logging, one that rewrites a reversal, one that rewrites an unstated run, one that resolves an ambiguity it should refuse — and asserts each is caught. A shared suite nobody has proven can fail is a green light held up by nothing.

**A row that was written and then DELETED, which is the point of the meta-lock.** The first draft asserted the gate is not stateful across calls, on the theory that a shared `/g` regex carries `lastIndex`. Measured: `String.prototype.matchAll` **clones** the regex and leaves the original's `lastIndex` untouched, and `match` with `/g` resets it — so the mutant could not be built and the row could not fail. A check that cannot fail checks nothing ([memory: locks and gates are hypotheses]), so it was removed and the spec's docblock corrected to say the factories are for clarity, not because a shared instance breaks.
...
**Requirements:** [02w](02w-requirements-workspace.md) FR-SL-10 (new), FR-DI-1 (new, and the new *Discoverability* area) · **Design:** [04w](04w-design-shell.md#share-pages-stay-out-of-the-index-adr-w-084) (two new sections)

**The defect, measured against prod.** `/robots.txt`, `/sitemap.xml` and `/favicon.ico` all answered
404. `https://www.themathbible.com/…` answered **200 with a full copy of every page**, with no
canonical anywhere, so search engines see two sites splitting one site's signals. And the `/g/`
share pages were indexable while carrying a `<title>` the **uploader** chose — a way for anyone to put
their own words on a themathbible.com search result — plus a `<link rel="canonical">` pointing at a
URL *with a `#fragment`*, which search engines strip.

**Root cause.** The site root was never treated as a surface: `deploy/homepage/` held one file, and
the share page was written for chat-preview crawlers without asking what a *search* crawler does with
the same page.

**Decisions.**

1. **Allow every crawler, AI crawlers included** — the operator's ruling. Reach is the point of a free
   educational tool, and blocking training bots buys little while risking the answer bots that cite.
2. **`/g/` is kept out by `X-Robots-Tag: noindex`, NOT by `Disallow: /g/`.** A disallowed URL is never
   fetched, so its noindex is never read (Google can still list a blocked URL it finds through links),
   and a chat preview crawler that honours robots.txt would lose the preview #1374 exists for. The
   header is set **once at the top of `handleSharePage`**, so the page, the PNG and the 404 all carry
   it and no future early return can skip it; the page adds the `<meta name="robots">`. The fragment
   canonical is removed.
3. **One icon source.** `deploy/homepage/favicon.svg` — the site blue with a triangle and its
   **true incircle** (r = area/s = 6.26, computed, not eyeballed: it is a geometry site). The `.ico`
   (16/32/48) and the 180 px touch icon are regenerated by `scripts/render-icons.mjs` through Chromium
   and committed, because the homepage deploys as a plain copy. Builders fall back to the root icon
   until #1383 gives each its own.
4. **`www.` → apex is a Plesk setting and needs the operator's hands** (RUNBOOK, with the probe). A
   session may not change prod hosting settings; the repo records the step rather than pretending it.

**Not in scope, deliberately.** `llms.txt` (no major answer engine has confirmed reading it); the
page metadata and crawlable text of each builder (#1383); the `esc()` on the share page's id (#1380,
queued for the next round — this change does not touch that interpolation).

**Locks.** `server/__tests__/crawl-basics-1384.test.ts`: `noindex` on every `/g/` branch (page, PNG,

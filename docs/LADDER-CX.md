...
| 0b′ | `complexScopeOf()` — the letter families the lines DECLARE complex («u מספר מרוכב»), read from every line before any is lowered, and handed to each `parse()`. A declaration is a TYPE: order-independent, never an existing number. A line that reads without the scope and fails with it is a declared letter in a real slot (a size, an angle), reported as `declared-complex-real` ([ADR-CX-047](06d-decisions-complex.md#adr-cx-047)) | `cx0:parse` | yes → `complex-as-real` at 0e, naming the statement |
...
| 0d′ | `rootsMode()` — which of ADR-CX-005's three readings `X^n = …` has, ASKED from the names earlier lines mentioned, never stamped onto the fact ([ADR-CX-021](06d-decisions-complex.md#adr-cx-021)). Asked by the FOLD, not the parser: a stateless per-line grammar cannot know what earlier lines established, which is why the lowering lived on the retiring bridge and the shipped path had no solution sets ([ADR-CX-024](06d-decisions-complex.md#adr-cx-024)). An enumeration RESERVES its letter, and a later line mentioning it is refused, naming the equation. **Its pins are decided after every line is read**: a determined member claims the root it lies on, and the unstated names take the rest ([ADR-CX-044](06d-decisions-complex.md#adr-cx-044)) | `cx0:ref` | yes → the reserved letter |
| 0e | **the acceptance gate** — dry-run the trial line list, keep-prior on failure: a new statement may never break an earlier one. Reads the three signals a GIVEN produces (`contradiction`, `emptiedBy`, `unsatisfied`) and never `claims` — a wrong answer is marked ✗, not refused. Blames by asking which earlier line, removed, lets the new one in ([ADR-CX-023](06d-decisions-complex.md#adr-cx-023)) | `cx0:dryrun` | yes → `incompatible` naming the earlier statement; `impossible` when no earlier line explains it; `complex-as-real` naming the statement that uses a declared letter as a size or an angle, in either order (ADR-CX-047) |

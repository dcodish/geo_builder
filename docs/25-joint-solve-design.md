...
_Status: **APPROVED by operator 2026-07-25 ("go with your plan") and stages (a)+(b) BUILT the same day — with one measured amendment.** Stage (a): `src/engine/components.ts` (partition + `componentOf`/`minimalComponentOf` over the injected `allDrivableAncestors` walk) + trace observability + `components.test.ts`. Stage (b): `jointFirst` in step.ts — but placed as a **rescue tier BEHIND the recruit rungs**, not before them: the joint-BEFORE placement solved the ADR-103 figure yet moved a rectangle base vertex ~0.35 where the staged path moved it 0 (the stability-snapshot delta caught it) — the rungs embody least-movement, so they keep first claim, and the component tier adds pure rescue power at zero stability cost (997/997 engine tests, no deltas). The LM prerequisite is LANDED (`src/engine/solveLM.ts`, 18 characterization tests, the ADR-281 truss case measured: 5 LM iterations vs NM 20+ orders short at 5× budget). **Remaining: (b2) integrate solveLM's two-phase regularizer into the component solve so its minimal-movement matches the staged path → promote to joint-FIRST → stage (c) rung retirement, evidence-gated per §4.** The section below is the original design._ Slice S3.2 of [docs/24](24-foundation-hardening-plan.md); closes generator **G2** of [docs/23](23-architecture-review-2026-07.md) (the M2 solver-ownership class — ~10 new members in the 2.5 weeks after ADR-231, the heaviest still-generating class). Direction stated by [ADR-338](06-decisions.md#adr-338): "No mechanism owned a macro's defining constraints as one system … the joint minimiser already exists; the gap was purely in the **assignment layer**."_

...
- **V2 — Lane A (algebraic substrate). ✅ BUILT (2026-07-06, [ADR-3D-004](06b-decisions-3d.md#adr-3d-004)).** Axes; coordinate points; parametric lines; planes by equation (+ translucent patch rendering with a sensible drawn extent); derived feet/intersections; measure labels (length, area, angle, distances). **Gate:** 2022-Q2's full constructive chain reproduces (angle-45° pins `a` with its two branches; foot B; line ℓ echoed in parametric form; foot C; △ABC area verifies). **Gate met** — full chain He+En; membership selects the branch, wrong answers/impossible angle refuse honestly; ⟂ knee marks + patches + axes render; typed parametric LINES deferred to V3 (2024-Q2 needs them), measure display commands folded into scalar claims for now.
- **V3 — parameters + branch givens. ✅ BUILT (2026-07-06, [ADR-3D-006](06b-decisions-3d.md#adr-3d-006)).** Shared symbolic coefficients as free DOFs pinned by relations; sign/branch givens; the honesty rules (over-constrained, "no value of m satisfies…"). **Gate:** 2024-Q2 reproduces (m pinned by ⊥; cut point A shown; the ∦-for-every-m fact expressible as a probe). **Gate met** — typed parametric line (LinExpr components incl. `(m+6)z`), ⟂ pin via minima-scan root-finding (a non-negative residual has no sign change), never-parallel as a family-level claim, parallel cut-point refused honestly, on-line membership carries the ד investigation; sign branch givens on point components deferred to V4 (they belong to the pivot).
- **V4 — the pivot. ✅ BUILT (2026-07-06, [ADR-3D-007](06b-decisions-3d.md#adr-3d-007)).** Coordinate injection onto a gauge-free Lane-G figure (gauge solve + leftover symbolic components + branch selection). **Gate:** 2020-Q2 ג and 2023-Q2 ג–ה complete — **all four corpus questions reproduce end-to-end**, the doc-09 "Q1–Q7 reproduced" moment for this tool. **GATE MET — the milestone reached:** LM similarity+dims solve (numeric, D3-compliant), partial/symbolic pins, sign-given branch selection over the mirror pair, plane-through points + the ד intersection line; uninjected DOFs stay free (the 2020 height resamples while K stands still). 2023-ה (find-a-plane) deferred as planned.
- **V5 — breadth + polish. ✅ BUILT (2026-07-06, [ADR-3D-008](06b-decisions-3d.md#adr-3d-008)).** Catalog filled + example chips; save/load (`.geo3.json`, schema-versioned); image export; DOF cue (modulo the 7-DOF gauge); LLM fallback live behind the proxy; widen the corpus to ~10 more Q2s (806/807 papers incl. purely-geometric ratio-proof questions and skew-line distance) as scenario gates. **Gate:** the widened corpus green; deploy to `/space-builder/`. **Gate met** — 6 more exams read (10 total), 2019 fully wired as a gate (line-through, colon ratios, angle/ratio claims), right pyramid, catalog + guard, LLM fallback via the shared proxy's `tool:'3d'` switch, responsive canvas, PNG export, DOF cue. **The headline documented gap → V7: points defined by general vector combinations** (`DF⃗ = (k/2)DB⃗ + kDC⃗`, 4 of 6 new exams).
- **V6 — solids-trig block (deferred but IN SCOPE, D4). ✅ BUILT (2026-07-06, [ADR-3D-009](06b-decisions-3d.md#adr-3d-009)).** Cylinder/cone/sphere; spatial-trig measures (volumes, lateral areas, angle-in-solid asks) — the curriculum's block 5. Committed scope, not optional: it ships after V5, it just doesn't gate the v1 launch. **Gate met** against formula-sheet canonical numbers (the 572 corpus never uses these solids — they live in the 571 paper, validation filed): free-vs-stated sizes per ADR-052, sampled-outline rendering (front/back split circles, silhouettes, the sphere's exact orthographic silhouette), volume/lateral-area/surface π-claims with honest `no-such-solid`/`free-size-claim` refusals.
...
Sequencing rationale: V8-a is cheap and removes friction across every legacy exam; V8-b is the highest-frequency real gap; V8-c is the already-greenlit solver-core change (best done early while the solver is in focus, and it clears the one deferred modern exam); the rest descend by frequency, circles (V8-i) and the niche constraint (V8-j) last. **Progress: V8-a…V8-i ✅ (ADR-3D-018..023, 025, 028, 029; ADR-3D-024 = the tetrahedron keyword, 026/027 = prod-triage fixes + line↔plane angle). V8-h = common perpendicular of two lines + projection of a line onto a plane; V8-i = a circle in R³ (centre+plane+radius) tangent to a line, rendered as a projected ellipse; V8-j ([ADR-3D-034](06b-decisions-3d.md)) = a point on a segment positioned so a derived pyramid is right (apex above the base centre, closed-form t). **V8 is COMPLETE — every 2009–2024 exam's space/vectors INPUT is expressible.** Documented deferrals: the exact parametric ℓ/ℓ' forms (V8-h/V8-i) wait on a multi-line-naming rework; oblique parallelepiped + orthoscheme + the dihedral face↔base angle are low-frequency coord-expressible items. Deferred within: oblique parallelepiped + orthoscheme (V8-d); dihedral face↔base angle + in-face altitude (V8-e); a face-on default view for the flat z=0 figures (V8-g renderer follow-up). V8-g adds the 2-D vector lane — flat free-point polygons (triangle/quad/pentagon) modelled as solids whose dims are their vertex coords, so the free case verifies affine vector identities multi-sample and the metric case drives via the pivot; plus a triangle-altitude foot. RESUME: V8-h (common perpendicular of two lines + projection of a line onto a plane — 2010-Q3, 2012-חורף).**

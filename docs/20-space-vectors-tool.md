...
Sequencing rationale: V8-a is cheap and removes friction across every legacy exam; V8-b is the highest-frequency real gap; V8-c is the already-greenlit solver-core change (best done early while the solver is in focus, and it clears the one deferred modern exam); the rest descend by frequency, circles (V8-i) and the niche constraint (V8-j) last. **Progress: V8-a…V8-i ✅ (ADR-3D-018..023, 025, 028, 029; ADR-3D-024 = the tetrahedron keyword, 026/027 = prod-triage fixes + line↔plane angle). V8-h = common perpendicular of two lines + projection of a line onto a plane; V8-i = a circle in R³ (centre+plane+radius) tangent to a line, rendered as a projected ellipse. Both source their lines from through-lines; the parametric ℓ/ℓ' forms wait on a multi-line-naming rework. ONLY V8-j left (a point positioned so a derived solid becomes a right pyramid). Deferred within: oblique parallelepiped + orthoscheme (V8-d); dihedral face↔base angle + in-face altitude (V8-e); a face-on default view for the flat z=0 figures (V8-g renderer follow-up). V8-g adds the 2-D vector lane — flat free-point polygons (triangle/quad/pentagon) modelled as solids whose dims are their vertex coords, so the free case verifies affine vector identities multi-sample and the metric case drives via the pivot; plus a triangle-altitude foot. RESUME: V8-h (common perpendicular of two lines + projection of a line onto a plane — 2010-Q3, 2012-חורף).**

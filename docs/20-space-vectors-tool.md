...
Sequencing rationale: V8-a is cheap and removes friction across every legacy exam; V8-b is the highest-frequency real gap; V8-c is the already-greenlit solver-core change (best done early while the solver is in focus, and it clears the one deferred modern exam); the rest descend by frequency, circles (V8-i) and the niche constraint (V8-j) last. **Progress: V8-a…V8-f ✅ (ADR-3D-018..023). Deferred within: oblique parallelepiped + orthoscheme (V8-d); dihedral face↔base angle + in-face altitude (V8-e); the pure-2D-triangle builds of the G6 exams (2013-חורף/2014-קיץ-ב, no solid) reproduce end-to-end with V8-g's z=0 lane — V8-f delivers the vector-relation CAPABILITY (cos-angle · chained dot products · equal angles · 3-D bisector) on solid-bearing figures. RESUME: V8-g.**

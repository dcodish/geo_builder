...
> **Status (2026-07-16).** The full v1 pipeline is shipped and live in prod (engine, renderer, store, parser, LLM fallback, save/load, PNG + `.docx` exports; latest deploy tag `prod/2026-07-15-3`). Work is now **issue-queue-driven** ([ADR-265](06-decisions.md#adr-265); workflow: [22-workflow.md](22-workflow.md)) — day-to-day priorities are the open GitHub issues on `dcodish/geo_builder`, not this plan. Authoritative *current-state* sources: the **ADR log** ([06-decisions.md](06-decisions.md)), **[DEPLOY-LOG.md](DEPLOY-LOG.md)** (what is live), and **[PROJECT-MEMORY.md](PROJECT-MEMORY.md)** (session log).
>
> **Resume pointer.** Clear the two open **P1 honesty bugs** first — the figure reads correct but silently asserts something false: **#153** the arc-SUM equality «קשת AC + קשת BE = …» drops the `+` terms; **#145** a multiplicative length relation («DM/ME=BM/DM») commits a wrong `set-equal`. Then the P2/P3 queue by priority. (**#141**, twin tangents on one touch point, was closed 2026-07-16 — resolved by #142's distinct-touch fix, operator-verified.)
>

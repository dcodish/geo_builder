...
>
> **Resume pointer.** The two P1 honesty bugs are FIXED at the class ([ADR-335](06-decisions.md#adr-335), the compound-measure-relations PR): **#153** (arc-sum truncation) and **#145** (multiplicative truncation) now build correctly via `set-measure-sum`/`set-length-product`, with the structural `droppedCompoundRelation` gate as the permanent net — closing #154 and #144 too. Next: the P2/P3 queue by priority; the Q22 HG⊥AB exam-conclusion check is an operator play-gate item (see the PR notes). (**#141** closed 2026-07-16 — resolved by #142's fix, operator-verified.)
>

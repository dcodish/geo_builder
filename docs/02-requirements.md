...
- **FR-EN-18 (Should)** — **From an external point: two tangents, and multiple secants.** (a) **Two tangents** — "from point E outside circle O two tangents touch the circle at A and B" / "מנקודה E … שני משיקים … בנקודות A ו-B" builds both tangent lines from E, touching at A, B (computed as `circle O ∩ circle-on-diameter-OE`, so EA⟂OA, EB⟂OB, |EA|=|EB|). (b) **A further secant from the same point** — once E exists, "from E a line cuts the circle at C and D" adds another secant sharing E without moving it, so several secants/tangents can emanate from one external point. *(Limit: a brand-new external E for the tangents defaults to a standard external position — intended for a single circle near the origin.)*

...
| **S5** | **scene + render** — the polar substrate, the sequence spiral and partial-sum chain, multiplication-as-rotation, the value cycle + `n` stepper, regions. **The shell-adoption half was struck 2026-08-16** (operator ruling on #622): `AppFrame`, the switcher, enable/disable, the DOF cue, undo/redo, image export, the build stamp and the privacy note are built once in `shell/` by the unification programme (#648, ADR-W-018) for all four builders | each picture type renders for its corpus witness; the polar↔cartesian toggle proven display-only over the catalog |

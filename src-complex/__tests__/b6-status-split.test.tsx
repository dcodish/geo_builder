...
// #716: the readers take the product i18n's t — these locks assert the HEBREW rendering, through
// the real resources (bidi isolates stripped: the locks compare visible text), so the seam and
// the words are both covered.
...
describe('the contradiction half (strip material)', () => {
  it('null when the figure holds; the refusal line when it does not', () => {
    expect(v2Contradiction(deriveLines(['z1 = 3+4i'], 0, 0), t)).toBeNull();
    ...
    expect(v2Contradiction(bad, t)).toContain('✗');
  });
});

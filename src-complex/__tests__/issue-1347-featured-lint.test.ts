/**
 * #1347 ([ADR-W-074](../../docs/06w-decisions-workspace.md#adr-w-074) Am. 1) — EVERY Csrc-complex/App.tsxED
 ...
 *
 * The rule itself is `featuredShortfall` in the shared chrome; this file asks it about complex's own
...
for (const r of CATALOG as unknown as (Row & { category?: string })[]) {
  const k = 'all';
...
describe('#1347 — complex: a capped section shows CHOSEN examples, not file order', () => {
  ...
  it.each(capped)('«%s» features its full six', (section, rows) => {
    ...
    expect(
      ...
      `complex / ${section}: ${rows.length} rows, ${rows.filter((r) => r.featured).length} featured. A capped ` +
  ...
  it('the cap this lint is written against is the one the app passes', async () => {
    ...
    const src = fs2.readFileSync(new URL('../../src-complex/App.tsx', import.meta.url), 'utf8');

...
export interface ShareSubject {
  ...
  /** The product's real "a link arrived" path. True when it loaded. Async in 2-D, which folds
   *  the figure off the main thread, so every caller awaits. */
  open: (payload: string) => boolean | Promise<boolean>;
...
export async function shareLinkFaults(s: ShareSubject): Promise<string[]> {
  ...
  for (const [label, bad] of [
    ...
    check((await s.open(bad)) === false, `${label} was ACCEPTED as a shared figure`);

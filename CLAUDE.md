...
**Next step (resume here):** the **corpus is fully reproduced (Q1–Q7)** — the v1 construct vocabulary is complete (5a→Q1, 5b→Q2–Q4 [[ADR-019](docs/06-decisions.md#adr-019)], 5c→Q5–Q7 [[ADR-020](docs/06-decisions.md#adr-020)]). No new constructs remain. Remaining v1 work is cross-cutting: finish **5d** — the constraint phrasings deliberately deferred (a *single* angle bisector, and `parallel`/`perpendicular`/distance as constraints that *shape* the figure), plus free-point (2-DOF) drivers, on the generic mechanism of [ADR-014](docs/06-decisions.md); and **ADR-018 Stage 1** (pinned-vs-free points + a seeded DOF sampler for "show another configuration"). Then **Phase 6 (theorems)**. Optional polish: draw a bare/clipped line (visible tangent/bisector), right-angle & equal-length tick marks. Free-point **moves** are already supported ([ADR-011](docs/06-decisions.md)). **173 tests green, build clean** on branch `rebuild-foundation`.

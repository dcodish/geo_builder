...
- **`src/` app shell** — `main.tsx`, i18n (Hebrew default, RTL via `document.dir`), styles, and `App.tsx` now the **store-driven shell**: canvas + fact list (checkbox to include/exclude, click to highlight, × to delete) + undo/redo/clear + alternatives + language toggle. The text input is **disabled** (parser is Phase 4); a "quick facts" row drives the store so the loop is usable now. The **parser and real text input are not built yet** (Phase 4).

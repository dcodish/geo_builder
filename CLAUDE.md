...
The original implementation was a *shape-template matcher* that dead-ended (couldn't represent free points, points-on-objects, accumulating constraints, or alternative configurations). Rebuilding from scratch as a constructive engine. **Phases 1–4 complete (M1–M3: engine, renderer, store, parser — text box live). Phase 5a complete: parallelogram, general quadrilateral, arbitrary segment, line∩line — corpus Q1 reproduced. Phase 5b complete ([ADR-019](docs/06-decisions.md#adr-019)): a `Line` object (bisector/perpendicular/parallel/through, never rendered) + foot, midpoint, line∩line of two lines, point-on-extension, right-triangle — corpus Q2, Q3, Q4 reproduced from typed He/En utterances. 159 tests green. Phase 5 continues construct-by-construct (5c: circles → Q5–Q7; 5d: constraint-driven DOF).**

- **`src/engine/`** — the constructive engine: dependency-graph data model (`types.ts`, with `isGeoPoint` the single source of truth for point kinds), pure `geometry.ts` (incl. `solveParam`, a generic deterministic 1-D root finder, and `footOnLine`), `solve.ts` (constraint refs/residual/description — ADR-014), `applyCommand` reducer, topological `evaluate` (over-constraint detection; resolves `Line` objects and points in one interleaved sweep), and `step` (apply/keep-prior, `commandConflict`, free-point move, `cycleAlternative`, `branchCount`). Supported constructs: the polygon family (`square`/`rectangle`/`rhombus`/`trapezoid`/`parallelogram`/`quadrilateral`/`triangle`/`right-triangle`), point-on-segment (incl. extension `t>1`), `point-by-distances` (circle∩circle), `segment` (undirected/idempotent), `set-angle`; and the **Phase-5b `Line` layer** ([ADR-019](docs/06-decisions.md#adr-019)) — a `Line` object (`through`/`bisector`/`perpendicular`/`parallel`, resolved to `(anchor,dir)`, **never rendered** — scaffolding only) plus the points it produces: `line-intersection` (two lines, e.g. bisector∩bisector), `foot` (⟂ foot), `midpoint`, and the 5a 4-point `line-line-intersection` for through∩through. `set-angle` **drives a free DOF when any referenced point has one** (an on-segment `t` solved so the angle holds, branches as alternatives — ADR-012/ADR-014) and is a *check* when every referenced point is determined.
...
- **`src/parser/`** — the grammar parser (Phase 4): `parse.ts` is the deterministic, offline `utterance → command[]` boundary — bilingual (He/En), keyword-order-independent rules for the polygon family (triangle/right-triangle/square/rectangle/rhombus/parallelogram/trapezoid/quadrilateral), segment, point-on-segment, point-on-extension, point-by-distances, free point, line∩line, angle, and the **Phase-5b** bisector-meet compound, foot-of-perpendicular, and midpoint. Unmatched input returns `{ ok:false, reason:'not-handled' }` (where the Phase-7 LLM fallback escalates) — including, on purpose, a *single* bisector fact and the parallel/⟂ *constraints* (Phase 5d). `catalog.ts` is the user-facing reference **and coverage map** (drives the in-app **commands** panel): every v1 construct, grouped by `category`, each ✓ wired (clickable to try) or ○ not-yet with a `phase` tag — single source of truth, flip `supported` when a construct lands (a guard test asserts every supported example parses both locales). Tested in `phase4.test.ts`.
...
**Next step (resume here):** Phase **5c** — **circles**: circle (centre+radius), point-on-circle / inscribed vertex, chord & diameter, arc midpoint, line∩circle, tangent-at-a-point → reproduce corpus **Q5–Q7**. (Phase 5 is corpus-driven; each construct lands in the engine, renderer, and grammar in lock-step.) Then finish **5d** — the constraint phrasings 5b deliberately deferred (a *single* angle bisector, and `parallel`/`perpendicular` as constraints that *shape* the figure), plus distance residuals and free-point (2-DOF) drivers, on the generic mechanism of [ADR-014](docs/06-decisions.md); and **ADR-018 Stage 1** (pinned-vs-free points + a seeded DOF sampler for "show another configuration"). Free-point **moves** are already supported ([ADR-011](docs/06-decisions.md)). Phases 5a (Q1) + **5b (Q2–Q4, [ADR-019](docs/06-decisions.md#adr-019))** are done; **159 tests green**. The build is on branch `rebuild-foundation`.

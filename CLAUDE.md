...
- **`src/engine/`** — the constructive engine: dependency-graph data model (`types.ts`, with `isGeoPoint` the single source of truth for point kinds), pure `geometry.ts`, `applyCommand` reducer, topological `evaluate` (over-constraint detection), and `step` (apply/keep-prior, `commandConflict`, free-point move, `cycleAlternative`, `branchCount`). Tested in `phase1.test.ts` (11) + `phase5a.test.ts` (8). Supported constructs: `square`, `quadrilateral`, `parallelogram` (D derived = A+C−B), point-on-segment, `point-by-distances` (circle∩circle), `line-line-intersection`, `segment` (undirected/idempotent), angle-check. Constraint-*solving* (driving a DOF) is Phase 5d; today the angle constraint only validates.

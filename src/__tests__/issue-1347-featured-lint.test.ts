/**
 * #1347 ([ADR-W-074](../../docs/06w-decisions-workspace.md#adr-w-074) Am. 1) — EVERY Csrc/App.tsxED
 ...
 *
 * The rule itself is `featuredShortfall` in the shared chrome; this file asks it about 2-D's own
...
for (const r of COMMAND_CATALOG as unknown as (Row & { category?: string })[]) {
  const k = r.category ?? '?';
...
describe('#1347 — 2-D: a capped section shows CHOSEN examples, not file order', () => {
  ...
  it.each(capped)('«%s» features its full six', (section, rows) => {
    ...
    expect(
      ...
      `2-D / ${section}: ${rows.length} rows, ${rows.filter((r) => r.featured).length} featured. A capped ` +
  ...
  it('the cap this lint is written against is the one the app passes', async () => {
    ...
    const src = fs2.readFileSync(new URL('../../src/App.tsx', import.meta.url), 'utf8');

...
export function applyCommand(prev: Construction, cmd: Command, pos: Map<Id, Vec> = new Map()): Construction {
  ...
  switch (cmd.type) {
    ...
    case 'set-angle-acuteness':
      ...
      constraints.push({ type: 'angle-bound', vertex: cmd.vertex, ray1: cmd.ray1, ray2: cmd.ray2, ...(cmd.obtuse ? { min: 90, minStrict: true } : { max: 90, maxStrict: true }) });
      break;
    ...
    case 'set-angle-bound':
      constraints.push({ type: 'angle-bound', vertex: cmd.vertex, ray1: cmd.ray1, ray2: cmd.ray2, min: cmd.min, max: cmd.max, minStrict: cmd.minStrict, maxStrict: cmd.maxStrict });
      break;

    case 'set-length-bound':
      constraints.push({ type: 'length-bound', a: cmd.a, b: cmd.b, min: cmd.min, max: cmd.max, minStrict: cmd.minStrict, maxStrict: cmd.maxStrict });
      break;

...
describe('viewRelations / clearRelations', () => {
  it('computes the layer and caches it against the current facts array', () => {
    enter('rhombus ABCD');
  ...
  it('clearRelations turns the layer off', () => {
    enter('rhombus ABCD');
    s().viewRelations();
  ...
  it('a new fact makes the cached layer STALE (ref no longer matches → App hides it)', () => {
    enter('rhombus ABCD');
    s().viewRelations();
  ...
  it('"show another configuration" (a seed change) KEEPS the layer — ground truths are invariant', () => {

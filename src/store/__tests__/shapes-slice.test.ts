...
describe('detectShapes / clearShapes', () => {
  it('caches the result keyed to the current facts array', () => {
  ...
  it('a new fact makes the cached layer stale (facts ref differs)', () => {
    s().execute(square(), 'square ABCD', 'g1');
    s().detectShapes();
  ...
  it('clearShapes resets the layer to null', () => {
    s().execute(square(), 'square ABCD', 'g1');
    s().detectShapes();
  ...
  it('clear() also drops the shapes layer', () => {
    s().execute(square(), 'square ABCD', 'g1');
    s().detectShapes();
    s().clear();
    expect(s().shapes).toBeNull();

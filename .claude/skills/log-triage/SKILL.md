...
> **The one blind spot — don't paper over it.** When our grammar can't reproduce a step, the harness replays **what the LLM actually committed** (the `commands` field, issue #84), so the prefix stays faithful and the rest of the session remains real evidence. Where that isn't available — LLM steps logged before 2026-07-14, **all of 3-D** (#182), or a store `action` the replay cannot reproduce (edit / delete / show-another / slider; `clear`/`undo`/`redo` ARE followed since #189) — our figure is genuinely missing objects the student had, so any later failure may be *our artifact*. Those verdicts are marked `degraded`, never become LIVE rows, and land in **? UNVERIFIED**. If a `?` row matters to a recommendation, re-check it by hand (build the prefix yourself) — never promote it to a gap on the strength of the report alone.
